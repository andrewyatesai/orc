...
export function createBootstrapFatalExitBanner(): string {
  // Why: a partial Windows NSIS update can leave app.asar in place while a main
  // dependency is still mid-copy; the failing static require throws before the
  // source-level error guards load, and Electron's pre-import error dialog can
  // leave main resident and block the update's asar replacement. Force a clean
  // exit so the updater can retry instead of wedging.
  return `
;(() => {
...
export function createMainBootstrapPlugin(): Plugin {
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      // Why: the fatal-exit guard and source-level diagnostics both run after
      // Rollup's generated require prelude, too late to handle a missing
      // bootstrap dependency. Prepend the guard first so it is armed before any
      // require in the chunk can throw, then the diagnostics banner.
      mainChunk.code =
