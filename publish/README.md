...
> is no `CHECK_CMD` stage or snapshot version. What actually decides the public
...
- `config.sh` — staging remote, public-clone check, bounded optional check
  ...
  (staging/public stay `v<version>`). **Under mirror-head only the staging
  remote is read**; `CHECK_CMD_DEFAULT` and `VERSION_DEFAULT` are inert.
