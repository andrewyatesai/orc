...
describe('locale repair does not de-localize CJK UI labels', () => {
  ...
  it('translates the workspace status picker labels instead of pinning English', () => {
    const zh = (enValue, localeValue) =>
      repairTranslatedValue({ key: 'auto.components.sidebar.workspace.status.x', enValue, localeValue, locale: 'zh' })
    expect(zh('Play', '玩')).toBe('播放')
    expect(zh('Sky', '天空')).toBe('天蓝')
    expect(zh('Zinc', '锌')).toBe('锌灰')
    expect(
      repairTranslatedValue({ key: 'auto.components.x.y', enValue: 'Play', localeValue: '遊ぶ', locale: 'ja' })
    ).toBe('再生')
  })
  ...
  it('replaces the adjectival 的 color form with the bare noun, not English', () => {
    expect(
      repairTranslatedValue({ key: 'auto.components.x.y', enValue: 'Blue', localeValue: '蓝色的', locale: 'zh' })
    ).toBe('蓝色')
    expect(
      repairTranslatedValue({ key: 'auto.components.x.y', enValue: 'Neutral', localeValue: '中性的', locale: 'zh' })
    ).toBe('中性')
  })
