...
async function createSandbox() {
  const root = await realpath(
    await mkdtemp(path.join(tmpdir(), 'orca-localization-coverage-gate-'))
  )
  sandboxes.push(root)
...
describe('verify:localization-coverage rejects coverage regressions', () => {
  it('fails when new rendered text ships without going through the catalog', async () => {
    ...
    sandbox.rejects('rendered text that no locale catalog covers', 'jsx-text: "Recent workspaces"')
  })
  ...
  it('names the regression rather than failing silently', async () => {
    ...
    await sandbox.write(
      STATUS_FILE,
      `${CLEAN_STATUS_SOURCE}export const emptyState = { message: 'No sessions yet' }\n`
    )
