...
// The dev bundle patcher is shared by the electron-vite runner and the `orca-dev` CLI.
const devElectronApp = readFileSync('config/scripts/dev-electron-app.mjs', 'utf8')
...
describe('Electron Vite output contract', () => {
  ...
  it('gives the dev terminal daemon helper the TCC identity watched by Orca', () => {
    expect(devElectronApp).toContain('const helperBundleId = `${bundleId}.helper`')
    expect(devElectronApp).toContain("'Electron Helper.app',")
    expect(devElectronApp).toContain(
