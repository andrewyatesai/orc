...
//
// The fork's terminal engine is aterm (wasm, off the main thread), so there is no in-process
// xterm parse to use as a same-process yardstick here; the scans stand on their own agreement
// and timing numbers.
//
...
// Baseline: verbatim copy of the pre-#12681 hand-rolled strip (not exported).
...
// Mirrors the daemon mouse mirror's introducer pre-filter.
