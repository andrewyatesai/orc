// Deliberately carries no gate-coverage tag: gate-negative-coverage.test.mjs enumerates
// only `check:`/`verify:` package scripts, and a claim naming a gate outside that map
// fails its own audit. The rejections below are still the proof that `lint:react-doctor`
// fires, they just are not registered in that ledger.
