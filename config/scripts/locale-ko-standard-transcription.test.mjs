...
const catalog = JSON.parse(
  fs.readFileSync(
    new URL('../../src/renderer/src/i18n/locales/ko.json', import.meta.url),
    'utf8'
  )
)
...
describe('locale ko standard transcription', () => {
  ...
  it('rejects comments as 너무 길어, not 너무 커서', () => {
    ...
    const commentLeaves = leaves.filter((leaf) =>
      leaf.includes('안전하게 제출할 수 없습니다')
    )
  ...
  it('uses the standardized terminology for milestone, permission, and privacy', () => {
    ...
    expect(
      leaves.some((leaf) => leaf.includes('개인정보 보호 환경 안에서 실행됩니다'))
    ).toBe(true)
