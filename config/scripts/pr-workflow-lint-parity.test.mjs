...
// file so the parity check runs wherever it ships and skips cleanly where it does not.
const HAS_CI_PR_WORKFLOW = existsSync('.github/workflows/pr.yml')

describe('PR workflow lint parity', () => {
  it.skipIf(!HAS_CI_PR_WORKFLOW)('runs every `pnpm lint` step on pull requests', () => {
