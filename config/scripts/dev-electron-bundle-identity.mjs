...
// Why a constant display name rather than none: leaving the stock value makes every dev notification
// and System Settings > Notifications row read "Electron", indistinguishable from any other Electron
// app. A fixed name keeps that legible without reintroducing per-branch drift. Must stay in step with
// getDevInstanceIdentity().appName — the two together key the safeStorage service name.
export const DEV_BUNDLE_DISPLAY_NAME = 'Orca Dev'

/** Info.plist patches for the dev app bundle. Values must not vary per branch — see above. */
