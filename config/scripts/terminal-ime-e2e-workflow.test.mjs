...
const workflowPath = join(projectDir, '.github/workflows/terminal-ime-e2e.yml')
// Why: the fork runs no hosted CI, so this workflow is dropped — gate its assertions on the
// file itself; the runner/spec contracts below have no CI dependency and always run.
const HAS_CI_IME_WORKFLOW = existsSync(workflowPath)
...
describe('terminal IME e2e workflow', () => {
  it.skipIf(!HAS_CI_IME_WORKFLOW)('runs only on schedule or manual dispatch', () => {
  ...
  it.skipIf(!HAS_CI_IME_WORKFLOW)('installs native IBus Hangul and X11 input tools', () => {
    const runs = readWorkflow()
      .jobs['linux-x11'].steps.map((step) => step.run)
  ...
  it.skipIf(!HAS_CI_IME_WORKFLOW)(
    'runs deterministic boundaries before the real IBus suite',
    () => {
      const runs = readWorkflow()
        .jobs['linux-x11'].steps.map((step) => step.run)
        .filter((run) => typeof run === 'string')
