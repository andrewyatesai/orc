...
const HAS_CI_RELEASE_WORKFLOWS =
  existsSync(join(projectDir, '.github/workflows/release-cut.yml')) &&
...
describe('release E2E dispatch contract', () => {
  it.skipIf(!HAS_CI_RELEASE_WORKFLOWS)('dispatches tag-scoped E2E only after publication', () => {
    const releaseWorkflow = readWorkflow('release-cut.yml')
  ...
  it.skipIf(!HAS_CI_RELEASE_WORKFLOWS)(
    'keeps detached E2E identifiable and manually dispatchable by ref',
    () => {
  ...
  it.skipIf(!HAS_CI_RELEASE_WORKFLOWS)(
    'includes the paired-runtime web client in the shared E2E build artifact',
    () => {
      const buildStep = readWorkflow('e2e.yml').jobs.build.steps.find(
