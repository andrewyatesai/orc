...
// Why: the fork runs no hosted CI, so these workflows are dropped — gate on the asserted files;
// the composite-action contract below ships in .github/actions and always runs.
const HAS_CI_SIGNING_WORKFLOWS = [
  '.github/workflows/release-cut.yml',
  '.github/workflows/windows-signing-rehearsal.yml'
].every((relativePath) => existsSync(join(projectDir, relativePath)))

describe('Windows signing workflow contract', () => {
  it.skipIf(!HAS_CI_SIGNING_WORKFLOWS)(
  ...
  it.skipIf(!HAS_CI_SIGNING_WORKFLOWS)(
    'still installs SignPath when the cut ref predates the composite action',
    () => {
      const parsedWorkflow = readWorkflow('.github/workflows/release-cut.yml')
      const steps = parsedWorkflow.jobs.build.steps
      const stepNames = steps.map((step) => step.name)
  ...
  it.skipIf(!HAS_CI_SIGNING_WORKFLOWS)(
    'shares one SignPath module install path between release and rehearsal',
    () => {
