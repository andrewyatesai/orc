...
function patchesUnder(dockTitle: string, branch: string) {
  ...
  try {
    return getDevBundlePlistPatches()
  } finally {
...
describe('dev-electron-bundle-identity', () => {
  ...
  it('patches a stable bundle id under the shared dev prefix', () => {
    const byKey = Object.fromEntries(getDevBundlePlistPatches().map((p) => [p.key, p.value]))
    expect(byKey.CFBundleIdentifier).toBe(DEV_BUNDLE_ID)
    expect(DEV_BUNDLE_ID.startsWith('com.stablyai.orca')).toBe(true)
  })
  ...
  it('produces byte-identical patches on two different branches', () => {
    ...
    expect(patchesUnder('Orca: ALab Edition', 'main')).toEqual(
