...
describe('locale mobile new-badge override', () => {
  it('pins the badge to "new feature" wording, not "create new", through the repair path', () => {
    ...
    expect(repairTranslatedValue({ key: BADGE_KEY, enValue: EN_VALUE, localeValue: 'x', locale: 'ko' })).toBe('신규')
    expect(repairTranslatedValue({ key: BADGE_KEY, enValue: EN_VALUE, localeValue: 'x', locale: 'zh' })).toBe('新功能')
    expect(repairTranslatedValue({ key: BADGE_KEY, enValue: EN_VALUE, localeValue: 'x', locale: 'ja' })).toBe('新機能')
  })
