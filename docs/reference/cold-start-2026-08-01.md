...
1. **Fix the measurement first.** `markTerminalPaneBootPhase` latches to
   whichever pane reaches `boot-start` first; `first-terminal-frame` can only
   fire from a **visible** pane (a draw-suspended pane never presents). On a
   multi-tab restore those are different panes, so every derived phase between
   them subtracts two different objects. It reads coherently today only because
   the fixture makes `tabs[0]` both first-mounted and active.
   Fix: latch both ends to the visible pane — `pane.startRenderingSuspended` is
   already correct at build time.
   **Two prior attempts failed here.** One resolved the lane from a bare boolean
   ...
   none.
2. **Then trace the queue.** For the visible pane: how many builds were admitted
   before it, and how long it waited. `MAX_CONCURRENT_PANE_BUILDS = 2`, FIFO, and
   the first two slots are granted **synchronously**.
