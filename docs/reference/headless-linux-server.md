...
Git, Xvfb, and the AppImage tooling:

```bash
sudo apt-get update
sudo apt-get install -y curl file git jq xvfb zlib1g-dev
```
...
> **ALab builds:** ALab publishes no Linux AppImage release asset, so the staging
> download in [Upgrade steps](#upgrade-steps) does not apply verbatim. Rebuild from
> source as in [prerequisites](#ubuntu-and-debian-prerequisites) and stage
> `dist/orca-linux.AppImage` as `/opt/orca/orca-linux.AppImage.new` in place of the
> `curl` step. Every other step — verification, the rollback bundle, the atomic
> swap, and [Roll back](#roll-back) — applies unchanged.
