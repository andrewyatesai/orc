...
export function copyClipboardTextViaExecCommand(text: string, doc: Document = document): boolean {
  ...
  const onCopy = (event: ClipboardEvent): void => {
    ...
    // Why bubble + stop: the terminal engine's copy handler on the pane element
    // overwrites text/plain, and preventDefault alone does not stop it or any
    // later window-level handler.
