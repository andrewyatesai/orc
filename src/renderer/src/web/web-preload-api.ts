...
function createWebUiApi(): NonNullable<Partial<PreloadApi>['ui']> {
  ...
  return {
    ...
    // Why: the desktop seam reports a verified boolean; the browser's closest truth is
    // that a write path accepted the text — no usable path rejects rather than lying.
    writeClipboardText: async (text) => {
      await writeWebClipboardText(text)
      return true
    },
