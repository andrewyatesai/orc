...
describe('readRemoteServerInstallFailure', () => {
  ...
  it('bounds the status call by the deadline the caller passes', async () => {
    ...
    await readRemoteServerInstallFailure('server-1', { getUpdaterStatus }, install, runtime(), 4_000)
  ...
  it('costs no round trip once the replacement runtime answers', async () => {
    ...
    await expect(
      readRemoteServerInstallFailure('server-1', { getUpdaterStatus }, install, runtime('runtime-new'))
    ).resolves.toBeNull()
