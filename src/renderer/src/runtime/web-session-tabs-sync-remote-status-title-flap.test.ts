 ...
 * tabsByWorktree via resolveTerminalTabTitle, so tab.generatedTitle is the
 * VISIBLE label source that flaps.
 *
 ...
 *
 * Fork note: the title derivation runs the Rust orca-text core via wasm, so
 * this initialises it once from the committed renderer-tree bytes.
 */
...
import { toWebTerminalSurfaceTabId } from '../lib/git-wasm/terminal-surface-id'
...
// Derived once the wasm is ready (module-eval predates the beforeAll above).
let EXPECTED_GENERATED_TITLE: string
 ...
 * a Claude status OSC (pty-connection.ts, shouldOwnAgentStatusInRenderer =
 * runtimeEnvironmentId !== null): timing is undefined, so updatedAt is the
 * CLIENT's Date.now().
 *
...
describe('remote-paired pane: host snapshot mirror vs client byte-derived status/title (issue1-ui-flash)', () => {
  beforeEach(() => {
    EXPECTED_GENERATED_TITLE = deriveGeneratedTabTitle(CLIENT_PROMPT)!
  ...
  describe('single-authority invariant (fix gate — RED before the fence existed)', () => {
    it('agent status and visible tab label converge to one authoritative value under dual publication', () => {
      ...
      // that (whichever side is authoritative wins and sticks).
    ...
    it('a host surface without agentStatus does not delete the client-owned fresher live entry', () => {
      ...
      // Without the fence, buildMirroredAgentStatusPatch deletes the live entry
  ...
  describe('converged sequence pinned (deterministic post-fix)', () => {
