...
export function notifyHostOfMirroredEditorClose(
  ...
  // Record the close intent synchronously: a host snapshot landing before the dynamic import
  // resolves would otherwise flash the tab back. closeWebRuntimeSessionTab re-records it idempotently.
  ...
  // Deferred facade: the editor slice imports this helper during store creation, so a direct
  // web-runtime-session import would pull the store back in and trip cyclic init.
