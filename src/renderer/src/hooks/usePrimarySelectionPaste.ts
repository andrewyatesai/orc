...
// Why: the native follow-up paste lands in the terminal's hidden helper textarea
// (aterm keeps xterm's `.xterm` / `.xterm-helper-textarea` class contract); scope
// terminal-armed suppression to that surface so unrelated document pastes
...
export function usePrimarySelectionPaste(enabled: boolean): void {
  useEffect(() => {
    ...
    const suppressPendingPasteInput = (event: InputEvent | ClipboardEvent): void => {
      ...
      // native X11 paste that aterm would otherwise forward to the PTY — but only
      // for the terminal's own surface, never unrelated document pastes.
