...
function installApi(listRecipes: ReturnType<typeof vi.fn>): void {
  window.api = { ephemeralVm: { listRecipes } } as never
}
...
describe('useEphemeralVmRecipeOptions', () => {
  it('selects the initial recipe when it exists in the fetched list', async () => {
    const listRecipes = vi.fn().mockResolvedValueOnce(result(['repo', 'other']))
    installApi(listRecipes)
    const { container } = await render(<Harness repoId="repo-1" initialRecipeId="other" />)

    await vi.waitFor(() =>
      expect(container.querySelector('[data-testid="selected"]')?.textContent).toBe('other')
    )
    expect(container.querySelector('[data-testid="recipes"]')?.textContent).toBe('repo,other')
  })
