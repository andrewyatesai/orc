...
describe('settings navigation metadata', () => {
  ...
  it('keeps local runtime settings but hides remote Linux entries on a Windows desktop', () => {
    ...
    // Why: this fork ships a POSIX default-shell setting (#5097), so a Linux terminal host
    // still indexes 'Default Shell'; the Windows-only entries are what must disappear.
    expect(terminal?.searchEntries.some((entry) => entry.title === 'Default Shell')).toBe(true)
    expect(terminal?.searchEntries.some((entry) => entry.title === 'PowerShell Version')).toBe(
      false
    )
    expect(repoSection?.searchEntries.some((entry) => entry.title === 'Project Runtime')).toBe(
      false
