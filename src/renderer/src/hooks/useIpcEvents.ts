...
export function useIpcEvents(): void {
  useEffect(() => {
    ...
    const handleWorktreesChanged = async (
      ...
      // Why: the refresh queue passes the owner positionally, upstream's direct callers pass it in
      // options; read either or a host-scoped refresh silently falls back to the focused host.
      const refreshHostId = ownerHostId ?? options?.executionHostId
      await state.fetchWorktrees(
        repoId,
        options?.forceLocalOwner
          ? { forceLocalOwner: true }
          : refreshHostId
            ? { executionHostId: refreshHostId }
            : undefined
      )
      await useAppStore
        .getState()
      ...
      // here to never overwrite the active runtime's worktree state (#6628). Accepted
      // gap: the workspace-space entry survives until the next local-only rescan.
      if (
        options?.forceLocalOwner &&
      ...
      if (event.type === 'worktreesChanged') {
        void ensureRuntimeEventRepoKnown(environmentId, event.repoId).then(() =>
          worktreeChangeRefreshQueue.enqueue({
            repoId: event.repoId,
            // Why: the queue keys its per-host serialization off ownerHostId; executionHostId
            // would be dropped and collapse two hosts' refreshes onto one 'focused' lane.
    ...
    unsubs.push(
      window.api.worktrees.onChanged(
        async (data: {
          ...
          // instead of dropping the event or refreshing an unbound repo from the
          // wrong host — otherwise a CLI-created local worktree stays invisible in
          // the sidebar while a runtime is active (#6628).
