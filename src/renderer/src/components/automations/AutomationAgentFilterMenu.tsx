...
export function AutomationAgentFilterMenu({
  ...
  const agentLabel = translate(
    'auto.components.automations.AutomationAgentFilterMenu.agent',
    'Agent'
  )
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="scrollbar-sleek max-h-80 w-56 overflow-y-auto">
        <div className="p-1">
          <Input
            value={agentQuery}
        ...
        {activeCount > 0 ? (
          <>
            ...
            <DropdownMenuItem onSelect={() => onChange(EMPTY_AUTOMATION_AGENT_FILTER)}>
              {translate(
                'auto.components.automations.AutomationAgentFilterMenu.clear',
                'Clear filters'
              )}
            </DropdownMenuItem>
          </>
