...
describe('OnboardingInlineCommandTerminal command-finished forwarding', () => {
  ...
  it('prepares auto-paste against the created tab shell and re-prepares when it changes', async () => {
    vi.useFakeTimers()
    mocks.createTab
    ...
    try {
      ...
      // Why: the created tab's resolved shell (wsl.exe / powershell.exe) is
      // authoritative, not the requested powershell.exe override.
