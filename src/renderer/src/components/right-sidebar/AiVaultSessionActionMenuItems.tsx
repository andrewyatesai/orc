...
export function SessionActionMenuItems({
  ...
  onRevealLog?: () => void
  onOpenCwd?: () => void
  // Null when Delete is offered; a string is the tooltip explaining why it
  // isn't. Both absent when the caller doesn't wire delete at all (e.g. a
  // subagent menu), which hides the item entirely.
          ...
          {deleteBlockedReason ? (
            <Tooltip>
              <TooltipTrigger asChild>
                {/* A disabled item is pointer-events:none, so the trigger needs
                   this wrapper to receive hover. */}
                <div>{deleteItem}</div>
              </TooltipTrigger>
              <TooltipContent
