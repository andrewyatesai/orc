...
describe('startParkedTerminalByteWatcher', () => {
  ...
  describe('with an injected remote byte source', () => {
    ...
    it('never answers a DECSET 2031 subscribe from the injected source (#9993)', async () => {
      const source = createInjectedSource()
      const { dispose, ptyWrite } = await startWatcher({
        ptyId: REMOTE_PTY_ID,
        runtimeEnvironmentId: 'env-1',
      ...
      // The parser owns the one injected stream source; the 2031 responder is gone.
      expect(source.subscribeBytes).toHaveBeenCalledTimes(1)
