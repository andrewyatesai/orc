...
export function createRemoteRuntimePtyTransport(
  ...
  async function classifyRemoteStreamEnd(
    ...
    if (hostExitConfirmed) {
      ...
      handle = null
      remotePtyId = null
      // The end is now proven fatal — release the waiters onEnd left pending.
      setAttachmentUnavailable()
      clearPendingViewportClaim()
      emitRecoveryState()
    ...
    const nextStream = await getRemoteRuntimeTerminalMultiplexer(
      ...
      callbacks: {
        ...
        onEnd: () => {
          ...
          multiplexedStream = null
          multiplexedStreamHandle = null
          // A stream end is recoverable until proven otherwise, so waiters keep waiting
          // (the confirmed-exit branch of classifyRemoteStreamEnd releases them).
          setAttachmentReady(false)
          clearPendingViewportClaim()
          emitRecoveryState()
          ...
          if (isHostSessionMirror()) {
            ...
            // Why the policy: repeated same-handle end/reuse cycles must eventually stop on a replacement boundary.
            scheduleResubscribeAfterTransportClose(
