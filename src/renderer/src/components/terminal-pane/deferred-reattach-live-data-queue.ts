...
// Why: a growing Array with per-chunk filter/reduce/shift was O(n) per live frame during replay; ring the head and compact so enqueue stays linear.
export class DeferredReattachLiveDataQueue {
  ...
  // Why: reclaim the dropped prefix only once it dominates the backing array, so enqueue stays amortized O(1).
