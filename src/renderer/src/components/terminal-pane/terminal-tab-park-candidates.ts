...
export function buildTerminalTabColdParkCandidates(args: {
  ...
  // Why: stamp the tab the worktree was left on before the hidden pass ties them.
  ...
  return args.terminalTabs.map((terminalTab) => {
    ...
    if (isVisible || hasActivityTerminalPortal || args.shouldMeasureHiddenWorktree) {
      args.hiddenSinceByTabId.delete(terminalTab.id)
    } else if (!args.hiddenSinceByTabId.has(terminalTab.id)) {
