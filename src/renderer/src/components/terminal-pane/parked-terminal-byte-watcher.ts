...
export function startParkedTerminalByteWatcher(
  ...
  // Why: a caller that names the owner environment has injected the remote wire and parses
  // side effects here (ssh-pane-parking.md §3.3); otherwise the paired host forwards derived
  // facts over its environment event stream and this PTY never streams raw bytes.
  const rendererParsesRemoteBytes = (options.runtimeEnvironmentId ?? null) !== null
  const remoteRuntimePty = isRemoteRuntimePtyId(ptyId) && !rendererParsesRemoteBytes
  ...
  const processLiveData = (data: string): void => {
    ...
    // Why: empty pane callbacks — no terminal to deliver bytes to, the watcher wants only the parser side effects.
  ...
  // stream; those parked PTYs never consume multiplex slots or raw bytes.
  const unsubscribeByteParsers = processor === null ? null : subscribeBytes(processLiveData)
