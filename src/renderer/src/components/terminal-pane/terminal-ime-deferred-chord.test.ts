...
// is the sender that has to guarantee an exit — otherwise the wait and its listener outlive the
...
describe('createTerminalImeDeferredChordSender', () => {
  ...
  it('cancels every pending chord and detaches its listeners', () => {
    ...
    // One compositionend listener per defer, so cancelling both detaches two.
