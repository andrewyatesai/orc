...
import type { TerminalReconfirmationBinding } from './terminal-captured-input-dispatch'
...
import { showOsc52ClipboardBlockedToast } from './osc52-clipboard-toast'
...
type UseTerminalPaneLifecycleDeps = {
  ...
  // Why: must stay as wide as PtyConnectionDeps' detail union (reason OR #7596 lastCommand), else ptyDeps won't assign under strictFunctionTypes.
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (initialLayoutHadBuffers) {
        // Why: the engine owns the replayed bytes now; leaving the park-time copy in Zustand retains up to 512KB/leaf for the app's lifetime.
