...
export function installTerminalImeNativeTextForwarder(args: {
  ...
  const claimKeyEvent = (event: ImeNativeTextKeyEvent): boolean => {
    ...
    if (event.type === 'keyup') {
      ...
      // this press put nothing on the wire — swallowed by the input source. Suppressing
      // unconditionally would drop the kitty release for ordinary typing, because the
      // structural claim takes every printable keydown rather than the short punctuation
      // list the previous design claimed.
