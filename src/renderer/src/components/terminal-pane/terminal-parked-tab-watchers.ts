...
// Why: re-export so callers keep one import surface; the registry/reconciliation splits only serve import cycles and the max-lines budget.
export {
...
export type ParkedTerminalWatcherCoverageOptions = {
  ...
  /** settings.terminalRemotePaneParking !== false — the scoped kill switch for the ssh: and remote: classes. */
  remoteParkingEnabled?: boolean
}

const allowSnapshotBackedPty = (): boolean => true

type ParkRestorePolicyState = {
  settings: { terminalSshViewParking?: boolean; terminalRemotePaneParking?: boolean } | null
  /** Optional: reachability probing may not have populated the map yet. */
...
// Why: fact-mode watchers work for any pty whose bytes transit local main — SSH
// included — so watcher coverage follows the park-restore policy, not the
// stricter daemon-snapshot predicate. Two kill switches scope the same
// non-daemon classes (scoped remote-pane, C1 ssh); either one may veto.
function parkRestorePolicyFromState(
  state: ParkRestorePolicyState,
  remoteParkingEnabled?: boolean
): TerminalParkRestorePolicy {
  const remoteClassesEnabled =
    remoteParkingEnabled ?? state.settings?.terminalRemotePaneParking !== false
  if (!remoteClassesEnabled) {
    return { sshParkingEnabled: false, pairedRuntimeParkingEnvironmentIds: new Set() }
  }
  return {
...
export function canWatcherCoverParkedTerminalTab(
  ...
  // Why: the bare predicate is the older positional form still used by cold-activation call sites.
  opts?: ParkedTerminalPtyEligibility | ParkedTerminalWatcherCoverageOptions
): boolean {
  const options: ParkedTerminalWatcherCoverageOptions =
    typeof opts === 'function' ? { isPtyEligible: opts } : (opts ?? {})
  const isPtyEligible = options.isPtyEligible ?? allowSnapshotBackedPty
  const state = useAppStore.getState()
  const panes = resolveParkedTerminalPaneCandidates(tab, state)
  const restorePolicy = parkRestorePolicyFromState(state, options.remoteParkingEnabled)
  return (
    panes.length > 0 &&
    panes.every(
      (pane) =>
...
function startParkedTabWatchers(
  ...
  const restorePolicy = parkRestorePolicyFromState(useAppStore.getState(), remoteParkingEnabled)
...
function reconcileParkedTabWatchers(
  ...
  const state = useAppStore.getState()
  const expectedPanes = watchablePanes(worktreeId, tab, remoteParkingEnabled)
  ...
  if (reconciliation.restartAll) {
    ...
    startParkedTabWatchers(worktreeId, tab, restoreTitleOnRegister, remoteParkingEnabled)
    return
  }
...
function watchablePanes(
  ...
  const state = useAppStore.getState()
  const restorePolicy = parkRestorePolicyFromState(state, remoteParkingEnabled)
  return new Map(
