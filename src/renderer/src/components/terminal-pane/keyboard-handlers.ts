...
import {
  ...
  type TerminalReconfirmationBinding
...
type KeyboardHandlersDeps = {
  ...
  panePtyBindingsRef?: React.RefObject<Map<number, IDisposable & TerminalReconfirmationBinding>>
...
export function useTerminalKeyboardShortcuts({
  ...
  useEffect(() => {
    ...
    // Why: the pane's TUI opted into an enhanced key protocol; the policy stands its
    // readline-compat rewrites down so the app gets the real encoded chord.
    // The gate must cover kitty AND modifyOtherKeys, so read the aterm engine's
    // negotiated protocol rather than the kitty-only flag tracker.
    const isKittyKeyboardActivePane = (): boolean => {
      const manager = managerRef.current
      const activePane = manager?.getActivePane() ?? manager?.getPanes()[0]
      return activePane?.atermController
    ...
    // Why: a same-tick shortcut send has nothing to re-verify — the capture ladder
    // below only earns its cost when the send crosses an IME macrotask boundary.
    const sendShortcutInputNow = (pane: { id: number; leafId: string }, data: string): void => {
      if (paneTransportsRef.current.get(pane.id)?.sendInput(data) !== true) {
        return
      }
      noteShortcutInputSent(pane, data, panePtyBindingsRef?.current.get(pane.id))
    }
    ...
    const createCapturedInputSender = (
      ...
      return () => {
        ...
        if (sent) {
          noteShortcutInputSent(pane, data, capturedBinding)
        }
      }
    }
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      // Resolved once; every action branch below targets this same pane.
      const activePane = manager.getActivePane() ?? manager.getPanes()[0]
      const hasPendingImeComposition = hasPendingTerminalImeComposition(
        activePane?.terminal.element
      )
      ...
      const shortcutEvent = imeProcessEnter
        ? {
            ...
            shiftKey: e.shiftKey,
            repeat: e.repeat,
            // Why: this substitute stands in for a keystroke that IS mid-composition;
            // custom entries must still hard-gate on it.
            isComposing: true
          }
        : e
      ...
      if (action.type === 'sendInput') {
        ...
        sendShortcutInputNow(pane, action.data)
        return
      }
        ...
        if (
          ...
          if (pane && hasPendingTerminalImeComposition(pane.terminal.element)) {
            const action = resolveShortcutEvent({
              ...
              repeat: false,
              // Why: the guard above proved a composition is open; custom entries hard-gate on it.
              isComposing: true
            })
            if (action?.type === 'sendInput') {
              e.preventDefault()
    ...
    window.addEventListener('keypress', onNativeOnlyShortcutCompanion, {
      capture: true
    })
    window.addEventListener('keyup', onNativeOnlyShortcutCompanion, {
      capture: true
    })
    window.addEventListener('beforeinput', onNativeOnlyBeforeInput, {
      capture: true
    })
    ...
    return () => {
      ...
      window.removeEventListener('keypress', onNativeOnlyShortcutCompanion, {
        capture: true
      })
      window.removeEventListener('keyup', onNativeOnlyShortcutCompanion, {
        capture: true
      })
      window.removeEventListener('beforeinput', onNativeOnlyBeforeInput, {
        capture: true
      })
