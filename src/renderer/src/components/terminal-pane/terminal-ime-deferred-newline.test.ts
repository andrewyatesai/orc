...
describe('sendTerminalInputAfterComposition', () => {
  ...
  it('sends the newline one macrotask after compositionend so the glyph flushes first', () => {
    ...
    // Deferred a macrotask so the engine's own post-compositionend flush runs first.
    expect(send).not.toHaveBeenCalled()
  ...
  it('finishes from the captured composition transaction when deferral starts after compositionend', () => {
    const el = document.createElement('div')
    const send = vi.fn()
  ...
  it('waits for every overlapping captured composition transaction', () => {
    const el = document.createElement('div')
    const send = vi.fn()
