...
describe('createRemoteRuntimePtyTransport', () => {
  ...
  it('propagates pty-exit for stale non-mirror remote terminals', async () => {
    ...
    // Why (#9994): a stale handle now re-derives the pane from the host first; the
    // host withdrawing it is what authorizes retirement.
    runtimeCall.mockImplementation(async (request: { method: string }) =>
      request.method === 'terminal.resolvePane'
        ? { ok: false, error: { code: 'terminal_not_found', message: 'terminal_not_found' } }
        : { ok: true, result: {} }
    )
  ...
  it('rebinds a stale non-mirror pane to its host replacement instead of exiting', async () => {
    const { createRemoteRuntimePtyTransport } = await import('./remote-runtime-pty-transport')
    const onPtyExit = vi.fn()
    const onPtyRebind = vi.fn()
    const transport = createRemoteRuntimePtyTransport('env-1', {
    ...
    // Why (#9994): a pane with host coordinates re-derives its handle; a stale error
    // is a re-mint, not a lifecycle exit, so the pane must survive it.
    ...
    expect(onPtyExit).not.toHaveBeenCalled()
    expect(onPtyRebind).toHaveBeenCalledWith(
      'remote:env-1@@terminal-replacement',
      'remote:env-1@@terminal-stale'
    )
    expect(transport.getPtyId()).toBe('remote:env-1@@terminal-replacement')
  })
  ...
  it('ignores a late gone error for a replaced handle instead of retiring the successor', async () => {
    ...
    // Why: the write must be in flight on the OLD handle before the stale error
    // arrives, otherwise recovery gating rejects it and the late-error guard never runs.
    await vi.waitFor(() =>
    ...
    await Promise.resolve()
    emitSnapshot(latestSubscribePayload().streamId, 'reconnected state')
  ...
  it('resubscribes instead of reporting exit when a stream end is not a confirmed host exit', async () => {
    ...
    runtimeCall.mockImplementation(async (args: { method: string }) => {
      ...
      if (args.method === 'terminal.resolvePane') {
        // Why (#9994): the resubscribe re-derives the pane; the host still owns this handle.
        return {
          ok: true,
          result: {
    ...
    const { streamId } = latestSubscribePayload()
    const subscribeCallsBefore = runtimeSubscribe.mock.calls.length
    const subscribeFramesBefore = subscriptionSendBinary.mock.calls.length
    ...
    await vi.waitFor(() =>
      expect(subscriptionSendBinary.mock.calls.length).toBeGreaterThan(subscribeFramesBefore)
    )
    emitSnapshot(latestSubscribePayload().streamId, 'resubscribed state')
    expect(onExit).not.toHaveBeenCalled()
    expect(onPtyExit).not.toHaveBeenCalled()
    expect(transport.getPtyId()).toBe('remote:env-1@@terminal-1')
    expect(transport.isConnected()).toBe(true)
  ...
  it('retries inventory and reattaches the same HUB handle after a stream ends', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => expect(subscribedTerminalHandles()).toEqual(['terminal-stable']))
      const oldStreamId = latestSubscribePayload().streamId
  ...
  describe('federated replay geometry (fed §2.4)', () => {
    ...
    async function attachedTransport(initialBaseY: number, cols = 80) {
      ...
      // Why: wait for THIS attach's subscribe frame, not merely any sendBinary call —
      // a previous test's transport can still emit into the shared mock and satisfy a
      // generic toHaveBeenCalled() before our subscribe lands.
      await vi.waitFor(() =>
        expect(
          subscriptionSendBinary.mock.calls
            .map((call) => decodeTerminalStreamFrame(call[0]))
            .some((frame) => frame?.opcode === TerminalStreamOpcode.Subscribe)
        ).toBe(true)
      )
      return {
        transport,
