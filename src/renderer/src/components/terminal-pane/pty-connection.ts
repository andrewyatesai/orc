...
import type { PtyConnectionDeps, SessionRestoredBannerDetail } from './pty-connection-types'
...
export function connectPanePty(
  ...
  if (terminalKeyTargetSupportsEvents) {
    terminalKeyTarget.addEventListener('keydown', onTerminalKeyDown, {
      capture: true
    })
  }
  ...
  const workspaceEnv: Record<string, string> = {
    ORCA_WORKSPACE_ID: deps.worktreeId
  }
  if (folderWorkspace) {
  ...
  const readProposedTerminalGrid = (): {
    cols: number
    rows: number
  } | null => {
    try {
  ...
  const runDeferredConnect = (): void => {
    ...
    const showSessionRestoredBanner = (detail: SessionRestoredBannerDetail = 'restored'): void => {
      // Why: the #7596 lastCommand payload is an ordinary 'restored' banner carrying
      // the re-run affordance, so it latches under the same reason.
      const reason: SessionRestoredBannerReason = typeof detail === 'string' ? detail : 'restored'
      ...
      sessionRestoredBannerShown = reason
      deps.onShowSessionRestoredBanner(pane.id, detail)
    }
      ...
      while (pendingReplayData !== null) {
        ...
        if (clearBeforeReplay || data.length > 0) {
          ...
          rememberReattachPayloadAgentSignal(data, {
            fullScreenReplay: clearBeforeReplay
          })
        }
    ...
    function salvageRendererQueriesFromDiscardedRestoreData(data: string): void {
      ...
      if (unansweredQueryData) {
        ...
        writePtyOutputToXterm(unansweredQueryData, true, {
          hiddenStartupRendererQuery: true
        })
      }
    }
    ...
    const finishReattachLiveDataDeferral = (
      ...
      if (deliveredDeferredChunks > 0) {
        ...
        flushTerminalOutput(pane.terminal, {
          maxChars: MAX_DEFERRED_REATTACH_LIVE_CHARS
        })
      ...
      const applyReattachPayload = async (): Promise<void> => {
        ...
        if (connectResult?.snapshot) {
          ...
          if (modelSnapshot) {
            ...
            rememberReattachPayloadAgentSignal(modelData, {
              fullScreenReplay: true
            })
  ...
  return {
    ...
    dispose() {
      ...
      releaseHiddenRendererPtyDelivery()
      if (terminalKeyTargetSupportsEvents) {
        terminalKeyTarget.removeEventListener('keydown', onTerminalKeyDown, {
          capture: true
        })
      }
      clearPendingTerminalInputIntent()
