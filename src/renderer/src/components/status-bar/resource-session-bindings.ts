...
/**
 * Bulk-kill safety gate (#8459): only sessions still present in a fresh daemon inventory,
 * still unbound, AND provably agent-free may be killed. A stale popover list must never
 * decide the kill set — sessions bound since the popover opened are live.
 */
export function selectVerifiedOrphanSessions(
  freshSessions: readonly DaemonSession[],
  inputs: ResourceSessionBindingInputs
): DaemonSession[] {
  return selectUnboundDaemonSessions(freshSessions, inputs)
}
