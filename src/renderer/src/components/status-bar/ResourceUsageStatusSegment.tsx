...
export function ResourceUsageStatusSegment({
  ...
  const handleKillSession = useCallback(
    (session: UnifiedSessionRow): void => {
      // Why: only a provably ownerless, unbound row skips the prompt; bound rows,
      // unprovable owners, and the bulk kill all confirm.
  ...
  const runKillOrphansConfirmed = useCallback(async () => {
    ...
    try {
      ...
      // Read directly rather than via the hook's refresh, which returns void and
      // only writes state this closure cannot observe; a second inventory scan
      // here would double a list that can stall typing on large session sets.
      const freshSessions = await window.api.pty.listSessions().catch(() => null)
      if (freshSessions === null) {
        // Daemon unreachable: abort, and let the hook surface the error state.
        void refreshSessions()
        return
      }
      // Why this selector: it applies the same unbound + proven-ownerless predicate that
      // produced the advertised count, so the kill set can never widen past it (#8459).
  ...
  }, [resourceSessionBindings, workspaceSessionReady, refreshSessions, removeSessions, mountedRef])
