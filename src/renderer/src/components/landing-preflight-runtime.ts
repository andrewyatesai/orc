...
export function useLandingPreflightRuntime(): { preflightIssues: PreflightIssue[] } {
  ...
  useEffect(() => {
    ...
    // Why: poll only while the window is visible, so a backgrounded Landing screen stops spawning
    // gh/glab auth-status probes every 30s; the sibling effect force-refreshes on re-show.
