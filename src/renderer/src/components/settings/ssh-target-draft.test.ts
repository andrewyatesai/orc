...
describe('isSshTargetFormDirty', () => {
  ...
  it.each<Partial<EditingTarget>>([
    ...
    { systemSshConnectionReuse: false },
    { relayGracePeriodSeconds: '600' },
    // Why: flip from whatever EMPTY_FORM's default is so the case is a real diff
    // regardless of the relay-grace default (0 vs. bounded).
    { relayKeepAliveUntilReset: !EMPTY_FORM.relayKeepAliveUntilReset }
