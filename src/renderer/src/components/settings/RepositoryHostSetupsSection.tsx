        ...
        {projectHostSetups.map((setup) => {
          ...
          const setupHostLabel =
            runtimeOwnerEnvironmentId && executionHost?.kind === 'ssh'
              ...
              : (hostOptionById.get(setup.hostId)?.label ?? getExecutionHostLabel(setup.hostId))
          // Why: (id, hostId) is the setup identity — a bare id match highlights every
          // sibling host's row when the same setup id is cloned across hosts.
          const isCurrentSetup =
            setup.id === selectedProjectHostSetup?.id &&
            setup.hostId === selectedProjectHostSetup?.hostId
