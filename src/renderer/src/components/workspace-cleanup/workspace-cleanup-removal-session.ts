...
export function useWorkspaceCleanupRemovalSession({
  ...
  // Why: progress arrives after the click, so the ref synchronously rejects a second batch.
  ...
  const applyScanDefaults = useCallback(
    (candidates: readonly WorkspaceCleanupCandidate[], deletingWorktreeIds: ReadonlySet<string>) => {
