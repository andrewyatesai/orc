...
function makeRepo(): Repo {
  return { id: REPO_ID, path: '/repos/repo-1', displayName: 'Repo 1', badgeColor: '#000000', addedAt: 0 }
}
...
async function renderOpenPalette(overrides: Partial<AppState>): Promise<void> {
  useAppStore.setState({
    ...
    repos: [makeRepo()],
    worktreesByRepo: { [REPO_ID]: [makeWorktree('wt-alpha', 'Alpha workspace'), makeWorktree('wt-beta', 'Beta workspace')] },
...
function getTabRowIds(): string[] {
  return [...testContainer.querySelectorAll<HTMLElement>('[data-command-item^="workspace-tab:"]')].map(
