...
describe('LinearIssueWorkspace header actions', () => {
  it('exposes copy, open-on-Linear, and a labeled start-workspace control', () => {
    ...
    // Fork wires start-workspace to handleUseIssue (no attachedWorkspace dropdown seam).
    expect(header).toMatch(/size="sm"[\s\S]*onClick=\{handleUseIssue\}/)
    expect(header).not.toMatch(/size="icon-sm"\s+onClick=\{handleUseIssue\}/)
  })
})
