...
/** Only the facade members this key handler drives, so any aterm terminal the
 *  preview builds satisfies it. */
type PreviewClipboardTerminal = Pick<
  AtermTerminalFacade,
  'attachCustomKeyEventHandler' | 'getSelection'
>

/**
 * Installs the popout preview terminal's ONE custom key handler (the aterm
 * facade holds a single attachCustomKeyEventHandler) covering copy/paste chords
 * and the IME native-text bypass. Plain Mod+V is left to the Edit-menu
 * accelerator, which reaches this window as ui:appMenuPaste — matching it here
 * too would paste twice.
 */
export function installPreviewClipboardShortcuts(args: {
  terminal: PreviewClipboardTerminal
  ...
  terminal.attachCustomKeyEventHandler((event) => {
    if (args.claimImeKeyEvent(event)) {
      // Why: bypass aterm's key encoder for native-text keydowns so the committed glyph survives via the input event.
      return false
    }
