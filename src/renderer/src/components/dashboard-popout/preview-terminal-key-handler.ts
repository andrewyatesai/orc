...
/**
 * Installs the preview terminal's ONE custom key handler (the aterm facade keeps
 * a single attachCustomKeyEventHandler) covering copy/paste chords, the IME
 * native-text bypass, and the full pane shortcut policy. Plain Mod+V is left to
 * the Edit-menu accelerator, which reaches this window as ui:appMenuPaste —
...
export function installPreviewTerminalKeyHandler(args: {
  terminal: Pick<
    AtermTerminalFacade,
    'attachCustomKeyEventHandler' | 'getSelection' | 'scrollToTop' | 'scrollToBottom'
  >
  ...
  terminal.attachCustomKeyEventHandler((event) => {
    if (args.claimImeKeyEvent(event)) {
      // Why: bypass the engine's kitty encoder for native-text keydowns so the committed glyph survives via the input event.
      return false
    }
    ...
    switch (action.type) {
      ...
      case 'switchInputSource':
        // Why: the OS owns this chord — block the engine encoder without preventing the default.
        nativeOnlyShortcutTracker.armKeyDown(event)
        event.stopImmediatePropagation()
        return false
      // Why: pane-scoped chords have no target in a preview dialog. Swallow them
      // — a pane never sends these bytes to the shell, and the engine would encode
    ...
    // Why: an action not classified above is not ours to swallow — let the engine encode it.
    return true
  })
