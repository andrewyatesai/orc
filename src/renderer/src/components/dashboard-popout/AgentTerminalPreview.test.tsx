...
// The aterm engine behind the facade: its input sink carries the engine's own
// auto-replies alongside real keystrokes, so the tests drive it directly.
const controllerHarness = vi.hoisted(() => ({
  routeEngineInput: null as ((data: string) => void) | null,
  routePaste: null as ((text: string) => void) | null,
...
vi.mock('@/lib/pane-manager/aterm/aterm-terminal-facade', () => ({
  createAtermTerminalFacade: (deps: { options: Record<string, unknown> }) =>
    new (class {
      cols = 80
      rows = 24
      buffer = { active: { cursorY: 0, type: 'normal' } }
      ...
      // The real facade routes paste/input through its onData emitter, which is
      // the preview's host-synthesized-input seam.
      ...
      get __customKeyEventHandler(): ((event: KeyboardEvent) => boolean) | null {
...
vi.mock('@/lib/pane-manager/aterm/aterm-pane-renderer', () => ({
  ATERM_RENDERER_FONT_PX: 14,
  createAtermPaneController: (
    _container: HTMLElement,
    onInput: (data: string) => void,
    _onResize: (cols: number, rows: number) => void,
    onPaste: (text: string) => void
  ) => {
    controllerHarness.routeEngineInput = onInput
    controllerHarness.routePaste = onPaste
    const controller = {
      element: document.createElement('div'),
      textarea: document.createElement('textarea'),
      // 0-sized cells: the preview then leaves the container box alone, so the
      // grid-claim test can install its own measured .xterm-screen.
      cellSizeCss: () => ({ width: 0, height: 0 }),
      updateTheme: vi.fn(),
      reapplyEngineSettings: vi.fn(),
      scheduleDraw: vi.fn(),
      dispose: vi.fn()
    }
    controllerHarness.instances.push(controller)
...
vi.mock('@/components/terminal-pane/terminal-user-input-signal', () => ({
  subscribeToTerminalUserInput: (_terminal: unknown, _target: unknown, listener: () => void) => {
...
describe('AgentTerminalPreview', () => {
  ...
  beforeEach(() => {
    ...
    controllerHarness.routeEngineInput = null
    controllerHarness.routePaste = null
  ...
  afterEach(() => {
    cleanup()
    // Why: the fake-timer specs never restore, and a later spec's waitFor stalls forever on them.
    vi.useRealTimers()
    vi.clearAllMocks()
  })
  ...
  it('routes signaled user input while a live write parses and drops parser replies', async () => {
    ...
    await waitFor(() => expect(controllerHarness.routeEngineInput).not.toBeNull())
    ...
    act(() => {
      ...
      controllerHarness.routeEngineInput?.('k')
      controllerHarness.routeEngineInput?.('\x1b[1;1R')
    })
  ...
  it('keeps the existing terminal visible while a resync snapshot is captured', async () => {
    ...
    await waitFor(() => expect(terminal.clear).toHaveBeenCalledTimes(1))
  ...
  it('creates the emulator with the user terminal appearance, not the bare defaults', async () => {
    storeState.settings = {
      terminalFontSize: 18,
      terminalCursorStyle: 'bar',
      terminalCursorBlink: false,
      terminalScrollSensitivity: 4
    }
    render(<AgentTerminalPreview ptyId="pty-1" />)
    await waitFor(() => expect(terminalHarness.instances).toHaveLength(1))

    // Why: the engine reads these off the facade's options bag, so seeding the
    // pane-free defaults would render the preview at 14px with a block cursor
    // while the pane it mirrors honours the setting.
    const options = terminalHarness.instances[0]!.options
    ...
    expect(options.scrollSensitivity).toBe(4)
  })

  it('withholds the kitty advertisement for a local Windows ConPTY agent pane', async () => {
    render(
    ...
    // Why: several local ConPTY CLIs read the advertisement but can't decode
    // CSI-u (#2434) — the engine reads this once at construction.
    const options = terminalHarness.instances[0]!.options
    expect(options.vtExtensions).toEqual({ kittyKeyboard: false })
