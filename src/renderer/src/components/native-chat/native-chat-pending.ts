 ...
 * the run starts at (a newer queued prompt must not be pruned against an older
 * row). A missing message boundary falls back to send time: a fuzzy match must
 * never reach further back than an exact one.
 */
