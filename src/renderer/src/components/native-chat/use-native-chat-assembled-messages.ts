...
/** Keeps transcript assembly off the status-only render axis. A discarded render
 *  clones the committed assembler instead of mutating it, so React re-runs of a
 *  memo it never commits can't drift the cache (the assembly-purity invariant). */
