 ...
 *
 * Fork note: the seed labels carry a version number (`Opus 4.8`) where upstream
 * seeds a bare family, so bare version tokens are dropped from the required set
 * — otherwise a frame reporting a newer release (`Opus 5`) would fail to resolve
 * to its family. The `[1m]`-style variant tokens still gate, so precision holds.
 */
