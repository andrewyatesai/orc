...
// Why: the fork ranks in orca-git wasm; keep the mount-gating probe hermetic so
// it measures only the content gate, never wasm readiness.
vi.mock('@/lib/git-wasm/quick-open', () => ({
  createQuickOpenIndex: () => ({
    rank: () => [],
    exactMatches: () => ({ paths: [], basenames: [] }),
    fileCount: 0
  })
}))

vi.mock('@/lib/git-wasm/git-line-stats', () => ({
  isGitWasmReady: () => true,
