  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1">
        <CounterPill label={translate('alab.mission.done', 'done')} value={run.tasks.completed} tone="good" />
        <CounterPill label={translate('alab.mission.failed', 'failed')} value={run.tasks.failed} tone="bad" />
        <CounterPill label={translate('alab.mission.blocked', 'blocked')} value={run.tasks.blocked} tone="bad" />
        <CounterPill
  ...
  const ordered = useMemo(
    () =>
      [...runs].sort((left, right) => {
        ...
        return byAttention !== 0 ? byAttention : (right.created_at ?? '').localeCompare(left.created_at ?? '')
      }),
    [runs]
  )

  return (
    <div className="flex h-full flex-col gap-2 overflow-y-auto p-2" data-testid="alab-mission-strip">
