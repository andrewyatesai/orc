...
export function runWorktreeDelete(worktreeId: string, options: WorktreeDeleteOptions = {}): void {
  ...
  if (!target || instanceChanged) {
    ...
    // miss here is a routing gap, not a stale list, and must not claim the workspace is gone.
...
export function runWorktreeBatchDelete(
  ...
  // Why: a pinned identity that re-paired or vanished fails the whole batch closed, with feedback.
  if (!targets) {
    showWorkspaceListChangedToast()
    return false
  }
