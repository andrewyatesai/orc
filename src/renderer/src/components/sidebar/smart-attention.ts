...
/**
 * One terminal tab's contribution to an attention resolution: its hook entries, plus the title
 * heuristic for panes no fresh hook covers. Gated on `tabHasLivePty` so a slept tab's stale
 * working-pattern title can't leak through. Extracted so Cmd+J's recent rows can reuse the exact
 * per-tab resolution the sidebar's `buildAttentionByWorktree` runs.
 */
