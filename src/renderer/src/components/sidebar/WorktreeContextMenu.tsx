...
const WorktreeContextMenu = React.memo(function WorktreeContextMenu({
  ...
  const handleDelete = useCallback(() => {
    ...
    // Why: snapshot the exact target identity at click time so the delayed dispatch
    // acts on the row the user chose, not whatever re-paired into its id meanwhile.
    ...
    // Why: Delete can remove the active row and remount the sidebar, so dispatch the
    // intent after menu close for the same reason as Sleep above.
