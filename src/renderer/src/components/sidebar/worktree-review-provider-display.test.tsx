...
describe('ReviewIcon', () => {
  ...
  it('does not paint a manual-blocked GitLab pipeline like a passing one', () => {
    ...
    // Why: which needs-attention tone is mappers.ts's call (the fork maps manual to `failure` for
    // GitHub ACTION_REQUIRED parity, upstream to `pending`) — pin that it is one of them, not green.
    expect(blocked).toMatch(/text-(amber-500\/85|rose-500\/85)/)
