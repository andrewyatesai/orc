// Fork: the job → check-row mapping is wasm-backed, so init the real Rust path.
...
function gitLabJobs(...statuses: string[]): PRCheckDetail[] {
  ...
  // The pre-ready null can't happen here (wasm is initialised above); fail loudly if it ever does.
  if (!checks) {
    throw new Error('git wasm is not ready — the parity cases would silently degrade')
  }
  return checks
}
...
const PARITY_CASES: ParityCase[] = [
  ...
  // Why these read red rather than grey: a GitLab `manual` job without allow_failure BLOCKS the
  // pipeline, so the fork maps it to action_required in orca-core (bcc4fe0664) and the classifier
  // counts that as a merge blocker. An optional (allow_failure) manual job still maps to neutral.
  gitLabCase('GitLab blocking manual gate only', ['manual'], {
    state: 'failure',
    passed: 0,
    failed: 1,
    pending: 0,
  ...
  gitLabCase('GitLab blocking manual gate alongside a green pipeline', ['manual', 'success'], {
    state: 'failure',
    passed: 1,
    failed: 1,
    pending: 0,
  ...
  // The `running` pipeline string is left unpinned here: a blocking gate makes the jobs roll up red
  // while the pipeline itself still reports running — one of the knowingly-disagreeing shapes above.
  gitLabCase('GitLab blocking manual gate alongside a running job', ['manual', 'running'], {
    state: 'failure',
    passed: 0,
    failed: 1,
    pending: 1,
