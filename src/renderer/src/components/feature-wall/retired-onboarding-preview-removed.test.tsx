...
const SURVIVING_CODEX_VISUALS = ['WorkspacesAnimatedVisual.tsx', 'WorkbenchAnimatedVisual.tsx'] as const
...
describe('retired onboarding preview removed (#13489)', () => {
  it('detects a source that still reaches for the retired preview glyphs', () => {
    ...
    expect(reachesRetiredPreview("import { CodexInlineIcon } from './feature-tour-preview-glyphs'")).toBe(true)
  })
  ...
  it('drops the retired preview localization namespaces', () => {
    ...
    expect((wall?.wall as Record<string, unknown> | undefined)?.FeatureTourWorkspaceCard).toBeUndefined()
    expect(
