...
describe('RemoteBrowserStreamRestartScheduler', () => {
  ...
  it('grows the delay per counted attempt, never by elapsed wall time, and then stops', async () => {
    ...
    // Literal on purpose: pinning the ladder against the constant under test makes the budget's
    // size and shape (how long the pane waits before handing control back) self-certifying.
