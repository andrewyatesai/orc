...
describe('BrowserPane webview preferences', () => {
  ...
  it('keeps a replacement viewport while rebuilding a parent-drifted webview (STA-3228)', () => {
    ...
    expect(replacementContainer.lastElementChild).toBe(ensuredWebview?.webview as unknown as Element)
  })
  ...
  it('marks explicitly allowed CLI pages before the guest attaches', () => {
    ...
    expect(ensuredWebview?.webview.getAttribute('preload')).toBe(BROWSER_WINDOW_CLOSE_ALLOWED_PRELOAD)
