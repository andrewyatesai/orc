...
// The fork reads command bodies through the orca-git wasm op boundary, which is
// inert in the test env; drive the body/agent split straight off the command.
vi.mock('@/lib/git-wasm/terminal-quick-commands', () => ({
  ...
  getTerminalQuickCommandBody: (command: TerminalQuickCommand) =>
    command.action === 'agent-prompt' ? command.prompt : command.command
}))

const terminalCommand: TerminalQuickCommand = {
  id: 'cmd-1',
  label: 'Git status',
...
function renderItem(command: TerminalQuickCommand = terminalCommand): {
