...
import { orcaDispatch } from '@/lib/git-wasm/orca_git_wasm.js'
import { resolveTabAgentFromSignals, useTabAgent } from './use-tab-agent'

// Fork: the session parse/repair moved to the Rust orca-config core (the shared
// TS zod schema was deleted); reach the same core main drives over napi.
function parseWorkspaceSession(raw: unknown): ParsedWorkspaceSession {
  return JSON.parse(
    orcaDispatch('workspace-session-schema', 'parseWorkspaceSession', JSON.stringify(raw))
