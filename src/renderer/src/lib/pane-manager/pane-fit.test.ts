...
describe('safeFitAndThen unmeasurable-pane retry', () => {
  ...
  it('runs the continuation when the reveal fit arrives on a different pane object', async () => {
    // Why: PTY connections hold a toPublicPane() wrapper while PaneManager iterates the
    // internal panes, so park and drain are never the same object. Anything keyed on the pane
    // identity is silently unreachable — the deferred bucket keys on the terminal instead.
    const pane = createPane({ rect: { width: 0, height: 0 } })
    pane.setDisplay('none')
    const continuation = vi.fn()
