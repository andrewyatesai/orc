...
describe('tab agent status index parity with the pre-index full-map scan', () => {
  ...
  it('splits live and completed panes of the same tab', () => {
    const map = {
      [`tab-1:${leafId(0)}`]: statusEntry(`tab-1:${leafId(0)}`, 'done', 'codex'),
      [`tab-1:${leafId(1)}`]: statusEntry(`tab-1:${leafId(1)}`, 'working', 'claude')
    }
    expect(resolveFocusedTabAgent(map, undefined, 'tab-1')).toBe('claude')
    expect(resolveFocusedCompletedTabAgent(map, undefined, 'tab-1')).toBe('codex')
  })
  ...
  it('ignores pane keys parsePaneKey rejects and empty maps', () => {
    ...
    expect(resolveFocusedTabAgent({}, undefined, 'tab-1')).toBeNull()
  })
