...
export async function launchAgentBackgroundSession(
  ...
  // A hidden run tab must never be store-visible without its PTY (#2989): the
  // PTY spawns against a reserved tab id, and the tab is created only once it lives.
  ...
  let ptyId = '',
    ...
    tab: ReturnType<typeof store.createTab> | null = null
  let exitHandled = false,
    eagerPtyBuffer: EagerPtyHandle | null = null,
    terminalOwnership: ReturnType<typeof bindAutomationTerminal> = null
  ...
  try {
    ...
    // Adopting into a gone worktree would strand a live PTY behind an unreachable tab.
    ...
    // Why: the launch config is registered at reserve time, so it outlives a failure that never made a tab.
    ...
    if (createdTab) {
      // Why: a cleanup close is not a user close — keep it out of the Cmd+Shift+T reopen stack.
