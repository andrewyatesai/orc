...
// The fork resolves a worktree's execution host from its indexed `hostId`
// (worktree-runtime-owner), so the mock seeds `worktreesByRepo` rather than a
// synthetic active-host field.
...
describe('AI Vault tab title sync', () => {
  ...
  it('resolves the request host from the worktree owner and its scope path', () => {
    const store = makeState({
  ...
  it('batches workspace scopes per host within the wire bound', () => {
    ...
    const groups = groupAiVaultTitleRequests(Array.from({ length: 65 }, (_, index) => request(index)))
