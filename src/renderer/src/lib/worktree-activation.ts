...
export function ensureWorktreeHasInitialTerminal(
  ...
  if (startup && setup?.waitForAgentStartup === true) {
    ...
    const sequenced = createSequencedSetupAgentCommands({
      ...
      platform,
      // Why: the recorded shell wins (it knows the WSL/Git-Bash executable); the configured
      // terminal shell is the fallback so Git Bash still gets POSIX delivery, not cmd.exe (#6896).
      shell: setup.shell,
  ...
  if (setup) {
    ...
    const setupCommand = {
      command:
        wrappedSetupCommandStr ??
        setup.command ??
        buildSetupRunnerCommand(setup.runnerScriptPath, setup.shell ?? terminalShellFamily),
      env: setup.envVars
    }
    if (mode === 'new-tab') {
      ...
      'runnerScriptPath' in issueCommand
        ? {
            command: buildSetupRunnerCommand(
              issueCommand.runnerScriptPath,
              issueCommand.shell ?? terminalShellFamily
            ),
