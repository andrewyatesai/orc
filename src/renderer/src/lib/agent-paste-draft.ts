...
/**
 * Why: activation creates the tab synchronously but the PTY spawn is async, and
 * "the tab has a PTY" is a separate state from "the composer accepts input". Bind
 * the readiness observer the instant the primary PTY id lands (store subscription,
 * not a poll) so the composer budget starts from PTY bind and a fast agent's first
 * frame is replayed rather than missed. Falls back to `null` if the PTY never
 * spawns within `spawnTimeoutMs`, keeping that failure distinct from a cold
 * composer that exhausts its own `readinessTimeoutMs`.
 */
