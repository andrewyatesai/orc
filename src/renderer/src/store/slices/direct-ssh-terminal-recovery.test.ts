...
function baseState(overrides: Partial<DirectSshTerminalBindingState> = {}): DirectSshTerminalBindingState {
  return {
    tabsByWorktree: {},
    ptyIdsByTabId: {},
...
describe('direct SSH authority ledger primitives', () => {
  ...
  it('prunes only same-target rows captured under a superseded authority', () => {
    ...
    const state = baseState({
      directSshPaneRetryHistoryByTabId: {
        ...
        otherTarget: {
          authority: { targetId: 'other', providerEpoch: 'x' as SshProviderEpoch, connectionGeneration: 9 },
          attemptedAt: [3]
        }
      }
    })
  ...
  it('matches a live binding only under the same authority and tab generation', () => {
    ...
    expect(liveBindingMatches(makeTab('t', 'target@@pty-1', { generation: 1 }), binding, authority())).toBe(
      true
    )
    expect(liveBindingMatches(makeTab('t', 'target@@pty-1', { generation: 2 }), binding, authority())).toBe(
      false
    )
    // a pending-activation pane with no ptyId still counts as bound
    expect(
      liveBindingMatches(makeTab('t', null, { generation: 1, pendingActivationSpawn: true }), binding, authority())
    ).toBe(true)
    expect(liveBindingMatches(makeTab('t', 'target@@pty-1', { generation: 1 }), binding, authority('other'))).toBe(
      false
    )
  })
})
