...
// Why: a burst of failing writes coalesces at dequeue time, so each mutation rolling back to its own predecessor
//      leaves a phantom mid-burst note in the store while disk still holds the pre-burst list. The floor must converge.
