...
export const createTerminalSlice: StateCreator<AppState, [], [], TerminalSlice> = (set, get) => ({
  ...
  hydrateWorkspaceSession: (persistedSession, options) => {
    // Why: populated inside the reducer but applied after it — transferring PTY ownership touches live panes.
    ...
    set((s) => {
      ...
      // Why: a scoped hydration must only re-own the PTYs of the workspaces it replaces.
