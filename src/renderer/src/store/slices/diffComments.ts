...
async function persist(
  ...
  if (scope?.type === 'folder') {
    ...
    // Why: owner-scoped writes must target the folder workspace's runtime, not whichever runtime is focused.
    ...
    // Why: an older paired runtime strips this optional field; treat a missing echo as a write failure so we roll back.
...
const lastPersistedByWorktree = new Map<string, DiffComment[] | undefined>()
...
function enqueuePersist(
  ...
  //      (hydration, worktree refresh, remote push), so the old floor predates that state and must be re-seeded.
  // Why: seed before the queue entry below, or `has` is always true and the floor is never seeded.
  const chainBroken = lastMutationNextByWorktree.get(worktreeId) !== mutation.previous
  if (!persistQueueByWorktree.has(worktreeId) || chainBroken) {
    lastPersistedByWorktree.set(worktreeId, mutation.previous)
    floorSeedEpochByWorktree.set(worktreeId, (floorSeedEpochByWorktree.get(worktreeId) ?? 0) + 1)
  }
  lastMutationNextByWorktree.set(worktreeId, mutation.next)
  const run = async (): Promise<void> => {
    ...
    const seedEpoch = floorSeedEpochByWorktree.get(worktreeId)
    ...
    try {
      ...
      stateList =
        scope?.type === 'folder'
          ? findFolderWorkspaceOwner(state, scope.folderWorkspaceId)?.diffComments
      ...
      const floor = lastPersistedByWorktree.has(worktreeId)
        ? lastPersistedByWorktree.get(worktreeId)
        : mutation.previous
      rollback(set, worktreeId, floor, mutation.next)
      throw err
    }
    ...
    if (floorSeedEpochByWorktree.get(worktreeId) === seedEpoch) {
      lastPersistedByWorktree.set(worktreeId, stateList)
    }
  }
  ...
  const cleanup = (): void => {
    if (persistQueueByWorktree.get(worktreeId) === next) {
      ...
      lastPersistedByWorktree.delete(worktreeId)
      lastMutationNextByWorktree.delete(worktreeId)
      floorSeedEpochByWorktree.delete(worktreeId)
    }
  }
...
function rollback(
  ...
  set((s) => {
    ...
    if (scope?.type === 'folder') {
      const target = findFolderWorkspaceOwner(s, scope.folderWorkspaceId)
      // Why: bail if the workspace is gone or a later mutation already replaced the array, else our stale `previous` would erase newer state.
...
export const createDiffCommentsSlice: StateCreator<AppState, [], [], DiffCommentsSlice> = (
  ...
  addDiffComment: async (input) => {
    ...
    try {
      ...
      // Why: enqueuePersist rolls back to the persisted floor itself on failure — callers must not roll back here.
  ...
  updateDiffComment: async (worktreeId, commentId, body) => {
    ...
    // Why: scope-aware read so a folder-workspace note resolves the same as a worktree note.
