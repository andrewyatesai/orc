...
describe('fetchWorktrees', () => {
  ...
  it('preserves legacy hostless local worktrees when a same-id runtime host refreshes', async () => {
    ...
    // A runtime-host refresh also stamps which Orca server owns the row.
    expect(store.getState().worktreesByRepo[repoId]).toEqual([
      local,
      { ...remote, runtimeOwnerEnvironmentId: 'env-1' }
    ])
  ...
  it('fetches the requested host when duplicate repo ids exist', async () => {
    ...
    await store.getState().fetchWorktrees('same-repo', { ownerHostId: 'local' })
  ...
  it('forceLocalOwner pins an unbound repo refresh to the local host and preserves remote rows under an active runtime (#6628)', async () => {
    ...
    expect(mockApi.worktrees.listDetected).toHaveBeenCalledWith(
      expect.objectContaining({ repoId: 'repo1', executionHostId: 'local' })
    )
    expect(runtimeEnvironmentCall).not.toHaveBeenCalled()
...
describe('fetchAllWorktrees hydration-time purge (design §4.4)', () => {
  ...
  it('refreshes same-id repos on separate hosts without clobbering sibling worktrees', async () => {
    ...
    expect(mockApi.worktrees.listDetected).toHaveBeenCalledWith(
      expect.objectContaining({ repoId, executionHostId: 'local' })
    )
    expect(runtimeEnvironmentCall).toHaveBeenCalledWith({
      selector: 'env-1',
      method: 'worktree.detectedList',
    ...
    expect(store.getState().worktreesByRepo[repoId]).toEqual([
      localWorktree,
      { ...runtimeWorktree, runtimeOwnerEnvironmentId: 'env-1' }
    ])
