...
export const createWorktreeSlice: StateCreator<AppState, [], [], WorktreeSlice> = (set, get) => ({
  ...
  fetchWorktrees: (async (
    ...
    try {
      ...
      // routing (and purging) against the remote runtime owner (#6628); the owner
      // guard keeps runtime/SSH-only repos from being redirected to local.
      const useLocalOwner =
        options?.forceLocalOwner === true && (hasLocalOwner || repoOwners.length === 0)
      // Why: events from a non-focused host name the owner so a duplicate repo id refreshes
      // that host's rows, not the focused host's; fork callers spell it `ownerHostId` (#6032).
      const ownerHostId = options?.ownerHostId ?? options?.executionHostId
      const hostId = useLocalOwner
        ? LOCAL_EXECUTION_HOST_ID
        : repoHostId(ownerState, repoId, ownerHostId)
      const setup = getProjectHostSetupForRepoHost(ownerState, repoId, hostId)
      const repoOwner = findRepoForHost(ownerState.repos, repoId, {
        hostId,
        settings: ownerState.settings
      ...
      const ownerSettings = settingsForRepoOwner(
        ...
        repoId,
        hostId,
        ownerWasMissingAtStart && (useLocalOwner || ownerHostId !== undefined)
      )
      const settings =
      ...
      const admitted = mergeFetchedWorktrees(set, {
        ...
        missingDirectSshOwnerReposSnapshot:
          ownerWasMissingAtStart && ownerHostId === hostId ? ownerState.repos : undefined,
        requestStartedWorktrees,
        setup,
        refresh
      })
  ...
  fetchWorktreeLineage: async (options) => {
    try {
      ...
      // Why: an explicit host owns the refresh; forceLocalOwner (no host) pins lineage
      // local so a local worktrees:changed under an active runtime refreshes local
      // lineage, matching the paired forced-local list refresh (#6628).
