  ...
  return {
    ...
    setAgentStatus: (paneKey, payload, terminalTitle, timing, routing, metadata) => {
      ...
      set((s) => {
        ...
        // Why: a same-state `done` write can flip completion eligibility (e.g. interrupted cleared)
        // without moving state or stateStartedAt, so the sort must re-run to age the row correctly.
