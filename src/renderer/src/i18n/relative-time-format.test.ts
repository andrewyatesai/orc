...
describe('getUiRelativeTimeFormatter', () => {
  ...
  it('formats in an on-demand locale', () => {
    mockIntlLocale.value = 'ja'
    expect(getUiRelativeTimeFormatter().format(-1, 'day')).toBe('昨日')
  })

  // The en-XA pseudo-locale is structurally valid, so the constructor never sees a tag Intl rejects.
  it('accepts the en-XA pseudo-locale and renders English words', () => {
    mockIntlLocale.value = 'en-XA'
    expect(getUiRelativeTimeFormatter().format(-1, 'day')).toBe('yesterday')
  })
})
