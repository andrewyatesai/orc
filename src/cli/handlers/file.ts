...
import { isWslUncPath, parseWslUncPath, toWindowsWslPath } from '../../shared/wsl-unc-paths'
 ...
 * Only the distro the caller is sitting in can name that Linux path, and only a
 * UNC root wants the rewrite — a POSIX root already matches, so rewriting it
 * would strand every absolute path.
 *
 * Backslash is a legal Linux filename character but a separator once the path
 * reads as UNC, so `a\b.ts` would relativize to a different file, `a/b.ts`. Such
 * a path has no Windows spelling; leave it to fail the match instead.
 */
