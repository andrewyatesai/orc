...
// The probe commands run on a worker thread; mock that boundary so the scanner
// tests drive command output directly without a built worker bundle.
...
describe('scanWorkspacePorts command timeout', () => {
  ...
  it('returns an unavailable scan when a probe command times out', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
...
describe('scanWorkspacePorts Linux /proc fallback chain', () => {
  ...
  it('falls back to ss when /proc/net/tcp is unavailable', async () => {
    ...
    rejectAllProcReads()
    runPortScanCommandMock.mockImplementation(async (command: string) => ({
      stdout:
        command === 'ss'
          ? [
              'LISTEN 0      511          127.0.0.1:5173       0.0.0.0:*',
    ...
    expect(runPortScanCommandMock.mock.calls.map(([command]) => command)).toEqual(['ss'])
  })
  ...
  it('falls back to lsof when /proc and ss are both unavailable', async () => {
    ...
    runPortScanCommandMock.mockImplementation(async (command: string) => {
      if (command === 'ss') {
        throw new Error('ss: invalid option -- "H"')
      }
    ...
    expect(runPortScanCommandMock.mock.calls.map(([command]) => command)).toEqual(['ss', 'lsof'])
  })
  ...
  it('does not launch fallback commands when /proc yields listeners', async () => {
    ...
    expect(runPortScanCommandMock).not.toHaveBeenCalled()
  })
  ...
  it('reports an empty scan instead of unavailable when /proc is readable and tools are missing', async () => {
    ...
    runPortScanCommandMock.mockRejectedValue(new Error('spawn ENOENT'))
