...
//
// This module stays electron-free: the only require('electron') lives in
// port-scan-command-worker-spawn.ts, so this class can never drag Electron into
// the worker entry's module graph (build-plugins/plain-node-entry-guard.ts fails
// the build on that text reaching a worker-thread entry).

// Why: the worker's own loop absorbs the spawn stall, so the client only needs
// a backstop for a wedged thread. Kept generous because a scan sits on the
...
export function resolveWorkerEntryPath(layout: WorkerEntryLayout): string {
  ...
  // the bundler's out dir, matching the shipped stt/warp/opencode workers.
