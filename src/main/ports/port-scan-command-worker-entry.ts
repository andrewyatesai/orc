...
// CreateProcessW off the Electron main-process UI thread. The client dispatches
// one request at a time, so this loop stays serial; imports must remain
// electron-free (the spawn helper holds the only require('electron')).
