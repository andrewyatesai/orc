...
// so the port scan only stays off the main-process UI thread while these
// main-thread modules spawn nothing themselves. Spawning belongs to
// port-scan-command-execution.ts, which only the worker entry imports.
