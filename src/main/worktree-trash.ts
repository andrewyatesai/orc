...
// `wt-<epoch-ms>-<nonce>`: the nonce keeps concurrent removals of same-named worktrees from colliding.
...
// Why: trash entries are always local (WSL/other-volume paths never reach the trash path), so a
// plain recursive delete is the host-correct removal here.
function removeTrashedTree(path: string): Promise<void> {
...
export function scheduleWorktreeTrashDeletion(trashPath: string): void {
  queuedTrashDeletions = queuedTrashDeletions.then(async () => {
    try {
      await removeTrashedTree(trashPath)
    } catch (error) {
...
export async function sweepStaleWorktreeTrash(
  ...
  for (const trashRoot of await collectExistingTrashRoots(workspaceRoots)) {
    ...
    for (const entry of entries) {
      ...
      try {
        await removeTrashedTree(join(trashRoot, entry))
        removed += 1
      } catch (error) {
        console.warn(`[worktrees] Failed to sweep leftover worktree at ${trashRoot}/${entry}`, error)
      }
    }
  }
  if (removed > 0) {
