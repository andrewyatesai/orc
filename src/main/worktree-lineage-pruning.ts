...
export function pruneLineageForMissingRepoWorktrees(
  ...
  // Why: one empty observation cannot prove every registered lineage edge disappeared at once, so a failed/degraded scan must not wipe lineage.
  if (
  ...
  for (const [childId, lineage] of Object.entries(worktreeLineage)) {
    if (worktreeIdBelongsToRepo(childId, repoPrefix) && !liveIds.has(childId)) {
