...
// Why: mirror the production seam — the writer stages via getCookieStagingDir, so use the real
// per-profile path builder (matching browser-cookie-import.test.ts) instead of a bare stub.
vi.mock('./browser-session-registry', async () => {
  const { orcaProfileCookieStagingDir } = await import('./cookie-import-staging-path')
  const { DEFAULT_LOCAL_ORCA_PROFILE_ID } = await import('../../shared/orca-profiles')
  return {
    browserSessionRegistry: {
      ...
      getCookieStagingDir: () => orcaProfileCookieStagingDir(DEFAULT_LOCAL_ORCA_PROFILE_ID)
    }
  }
})
