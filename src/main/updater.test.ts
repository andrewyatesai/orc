...
// Type-only aliases for the partial mocks below; erased at runtime so they
// cannot defeat vi.mock hoisting.
...
const {
  ...
  const shellMock = { openExternal: vi.fn(), showItemInFolder: vi.fn() }
...
describe('updater', () => {
  ...
  it('recovers quit-for-update state on sync quitAndInstall error event without killing PTYs', async () => {
    ...
    expect(sendMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        state: 'error',
        // Why: a pre-commit install failure is not fixed by restarting — except on
        // macOS, where quitting does re-stage the Squirrel update.
        message:
