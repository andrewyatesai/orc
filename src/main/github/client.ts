...
function deriveWorkItemCheckSummary(value: unknown): GitHubWorkItem['checksSummary'] {
  return summarizeProviderChecks(
    ...
    latestRollupEntriesByName(checkRollupEntries(value)).map((entry) => {
      if (typeof entry !== 'object' || entry === null) {
        return { status: '', conclusion: '' }
      }
      const raw = entry as Record<string, unknown>
      // Why: StatusContext reports `state` and carries no `status`; CheckRun reports both.
      const conclusion = String(raw.conclusion ?? raw.state ?? '')
      return {
        status: String(raw.status ?? ''),
        // Why: keep in sync with mapCheckConclusion — GitHub's STALE is a terminal failure the
        // provider-neutral classifier does not know (upstream #4605).
        conclusion: conclusion.toUpperCase() === 'STALE' ? 'failure' : conclusion
      }
    })
  )
}
...
// Why: a resolved OwnerRepo is required at the type level — gh api search/issues
// ignores cwd/--repo, so a repo-less issue query would search all of GitHub
// (#9202, #9553, #9660). Unresolved sources now yield an empty envelope instead.
...
async function listQueriedWorkItems(
  ...
  const issueFetch = (async (): Promise<PartialWorkItemsResult> => {
    ...
    // than leaking foreign issues (#9202).
    if (!issueOwnerRepo) {
      return { items: [] }
    }
