...
> {
  ...
  // Why: the runner injects `--hostname` from `exec.host` (applyGhHostToArgs),
  // so the #1715 GHES pinning happens once, at the single gh spawn site.
...
export async function runRest<T>(
  ...
  try {
    // Why: `--hostname` comes from `options.host` via the runner, so REST calls
    // pin the same GHES host as graphql without a second argv rewrite (#1715).
