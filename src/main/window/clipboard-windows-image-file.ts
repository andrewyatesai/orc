...
// Why: Explorer's FileNameW exposes only the first path even when its CIDA has
// multiple items, so the 'Shell IDList Array' count is the only clipboard signal
// that reveals the real selection size (#9640). Empty buffer = legacy
// single-file copy that predates CIDA; treat as OK.
...
// clipboard.readImage() cannot see a copied image file. Callers own the win32
// platform check and read both clipboard formats.
