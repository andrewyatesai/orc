...
describe('readWindowsClipboardImageFileAsPng', () => {
  ...
  it.each([
    ...
    ['non-image extension', fileNameW('C:\\Users\\alice\\notes.txt')],
  ...
  // Legacy single-file copies predate CIDA, so an empty 'Shell IDList Array'
  // must still paste instead of reading as a multi-file selection.
  it('treats an empty CIDA buffer as a legacy single-file copy', async () => {
    const filePath = 'C:\\Users\\alice\\shot.png'
    const png = Buffer.from([9, 8, 7])
    const handle = fileHandle(pngHeader())
    const openFile = vi.fn().mockResolvedValue(handle)

    await expect(
      readWindowsClipboardImageFileAsPng(
        { fileNameW: fileNameW(filePath), shellIdListArray: Buffer.alloc(0) },
        { createImageFromBuffer: vi.fn(() => image(png) as never), openFile }
      )
    ).resolves.toEqual(png)
