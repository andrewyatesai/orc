...
describe('packaged CLI assets', () => {
  ...
  // Why: `exec` replaces the shell with electron so a terminating signal reaches
  // the `orca serve` supervisor directly instead of orphaning it under bash.
  ...
  it.each([
    ...
    try {
      ...
      await expect(waitForListenerState(statePath)).rejects.toThrow('Invalid launcher listener state')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
