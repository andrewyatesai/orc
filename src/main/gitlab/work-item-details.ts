...
async function fetchPipelineJobPage(
  ...
  const { stdout } = await glabExecFileAsync(
    [
      ...
      // large pipelines; the first page matches the visible summary budget.
 ...
 * ~2.3x glab rate-limit waste. This path fetches MR detail (for
 * head_pipeline.id + sha), discussions, and the pipeline jobs+bridges (plus a
 * capped, throttled child-pipeline fan-out) — never diffs/reviewers/approvals.
 *
