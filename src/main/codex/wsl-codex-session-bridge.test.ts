...
describe('buildWslCodexSessionBridgeShellCommand', () => {
  ...
  it('keeps Linux-side shell variables verbatim for the --exec argv', () => {
    // Why raw: --exec passes argv to the guest untouched, so the script must
    // reach the shell with its own `$` expansions intact — no Windows-side escaping.
    const shellCommand = buildWslCodexSessionBridgeShellCommand({
      systemSessionsRoot: '/home/alice/.codex/sessions',
      managedSessionsRoot: '/home/alice/.local/share/orca/codex-runtime-home/home/sessions'
    })
    ...
    expect(shellCommand).not.toContain('\\$source_sessions_root')
  })
})
