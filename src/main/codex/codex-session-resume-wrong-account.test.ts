...
// account B's credentials while the UI still showed account A. These tests drive
// the real findTrustedCodexSessionResume + real listCodexSessionRolloutFiles-
// Incrementally so the production statSync/opendir paths are what refuse.
...
const { CodexSessionResumeHomeUnavailableError } = await import('./codex-session-resume-refusal')
...
describe('STA-4607 session resume under a briefly unreadable sessions tree', () => {
  ...
  // Why: a reviewer read the callback's guard as matching only the exact sessions
