...
async function resolveSelectedAccountCodexHomeForResume(
  ...
  try {
    ...
    // locked candidate and kept the source per-account home, resuming under
    // another account's credentials while the UI still showed the selected one.
    // Only a definitive absence means "not bridged here" (STA-4607).
