...
// Why: the probes under test derive every URL from the fork's feed slug, so the
// fixture responses must be keyed off the same helpers, not a hardcoded repo.
import { getReleasePageUrl, getReleasesAtomFeedUrl } from './updater-feed-endpoints'
...
function respondWithNotReadyRelease({
  ...
  const atom = `<feed>${publishingIncident.atomTags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="${getReleasePageUrl(tag)}"/><title>${tag}</title></entry>`
    )
    .join('')}</feed>`
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === getReleasesAtomFeedUrl()) {
