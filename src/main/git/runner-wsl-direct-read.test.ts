...
// Fork note (#13207 port): this fork already routes WSL reads through a non-login
// shell (`bash -c`, from the #9372 port), not the login shell (`sh -lc`) upstream
// falls back to. The direct-git (`--exec`) fast path is layered on top; when it is
// unavailable the exec fallback stays on the fork's non-login shell, while the stream
// fallback (via gitSpawn) keeps its login shell. Assertions below reflect that.
...
describe('WSL direct Git reads', () => {
  ...
  it('keeps the first read on the non-login shell while priming the fast path', async () => {
    await withPlatform('win32', async () => {
      let completeProbe: ((error: Error | null, stdout: string, stderr: string) => void) | undefined
      ...
      expect(execFileMock).toHaveBeenCalledTimes(2)
      // The env probe is primed on the login shell, while the read itself stays on
      // the fork's non-login shell until the fast path is available.
      expect(execFileMock.mock.calls[0]?.[1]?.slice(3, 5)).toEqual(['sh', '-lc'])
      expect(execFileMock.mock.calls[0]?.[1]?.[5]).toContain('ORCA_WSL_GIT_READ_ENV_V1')
      expect(execFileMock.mock.calls[1]?.[1]?.slice(3, 5)).toEqual(['bash', '-c'])
      completeProbe?.(
        null,
        `\0ORCA_WSL_GIT_READ_ENV_V1\0${LOGIN_ENVIRONMENT.path}\0${LOGIN_ENVIRONMENT.gitPath}\0${LOGIN_ENVIRONMENT.home}\0`,
        ''
  ...
  it('keeps unopted commands on the WSL non-login shell', async () => {
    await withPlatform('win32', async () => {
      seedWslGitReadEnvironmentForTests(DISTRO, LOGIN_ENVIRONMENT)
      succeedExecFile()
  ...
  it('keeps reads with custom Git environment policy on the non-login shell', async () => {
    await withPlatform('win32', async () => {
      seedWslGitReadEnvironmentForTests(DISTRO, LOGIN_ENVIRONMENT)
      succeedExecFile()
  ...
  it('invalidates a missing direct executable and retries through the non-login shell', async () => {
    await withPlatform('win32', async () => {
      seedWslGitReadEnvironmentForTests(DISTRO, LOGIN_ENVIRONMENT)
      execFileMock.mockImplementationOnce((_command, _args, _options, callback) => {
        const child = createMockChild()
  ...
  it('falls back and disables the fast path after another direct Git failure', async () => {
    await withPlatform('win32', async () => {
      ...
      expect(execFileMock.mock.calls[0]?.[1]).toContain('--exec')
      expect(execFileMock.mock.calls[1]?.[1]?.slice(3, 5)).toEqual(['bash', '-c'])
      expect(execFileMock.mock.calls[2]?.[1]?.slice(3, 5)).toEqual(['bash', '-c'])
    })
  })

  it('keeps the fast path when direct and fallback Git both report an expected failure', async () => {
    await withPlatform('win32', async () => {
      seedWslGitReadEnvironmentForTests(DISTRO, LOGIN_ENVIRONMENT)
      execFileMock
  ...
  it('retries a direct stream only when no stdout was delivered', async () => {
    await withPlatform('win32', async () => {
      ...
      expect(spawnMock.mock.calls[0]?.[1]).toContain('--exec')
      // Stream fallback goes through gitSpawn, which keeps the login shell.
