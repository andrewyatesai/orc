...
export class CodexRuntimeHomeService {
  ...
  syncForCurrentSelection(
    ...
    if (
      !this.writeRuntimeAuth(
        readFileSync(activeAuthPath, 'utf-8'),
        { owner: 'managed', accountId: activeAccount.id },
        // Why: #9582 — guard the replace on the baseline observed above so a live Codex PTY that
        // refreshed the shared runtime auth.json mid-pass loses nothing. An unreadable baseline has
        // nothing to compare against, so that case keeps the legacy unguarded replace.
        this.observedRuntimeAuthJson === RUNTIME_AUTH_FILE_UNREADABLE
          ? undefined
          : { expectedContents: this.observedRuntimeAuthJson }
      )
    ) {
      if (!isRetryAfterConcurrentAuthWrite) {
        ...
        this.syncForCurrentSelection(target, launchEnv, true)
        return
      }
      console.warn(
