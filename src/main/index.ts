...
import { setDefaultWslDistroOverride, gitExecFileAsync } from './git/runner'
...
async function renderTerminalPairingQr(pairingUrl: string): Promise<string | null> {
  // Lazy: qrcode (+ its pngjs dep, ~140 KB) is only reachable from mobile pairing, so it stays
  // out of the every-launch main-bundle parse for the majority who never pair a device.
      ...
      if (isAgentStatusHooksEnabled(managedHookStore.getSettings())) {
        void applyAgentStatusHooksEnabled(true, managedHookStore.getSettings(), {
          ...
          shouldContinue: (agent) => {
            ...
            return (
              isAgentStatusHooksEnabled(settings) && !settings.disabledTuiAgents.includes(agent)
            )
          }
        }).catch((error) => {
