...
// Why the extension: the cache stores payloads by reference, so #7596's field rides along untouched.
type ColdRestorePayload = CachedColdRestorePayload & {
...
export class DaemonPtyAdapter implements IPtyProvider {
  ...
  async spawn(opts: PtySpawnOptions): Promise<PtySpawnResult> {
    ...
    // Why counted across the whole spawn (retries included): getSessionLiveness must
    // report "no evidence" for the entire window the registry may not list this id yet.
  ...
  private async doSpawn(
    ...
    // Why re-checked here and not only in spawn(): this body is what withDaemonRetry
    // re-runs after a daemon respawn, and the session can be killed during that window —
    // the retry must reject rather than resurrect it (see releasePendingRespawnAdoptionLease).
    if (this.killedSessionTombstones.has(sessionId)) {
      throw new TerminalKilledError(sessionId)
    }
    ...
    const createOrAttach = async (
      ...
      const deliverSeedAndCreate = async (): Promise<CreateOrAttachResult> => {
        ...
        const segments = historySeedSegments
        const deliverInlineSeed = async (): Promise<CreateOrAttachResult> => {
          try {
            return await requestCreateOrAttach(segments.join(''), undefined)
          } catch (error) {
        ...
        // Why the namespace gate (not >= HISTORY_SEED_TRANSFER_PROTOCOL_VERSION): the fork's Rust
        // daemon is 10xx, which clears the public v30 constant while implementing none of the four
        // transfer RPCs — it takes the seed inline to the NDJSON line cap, as it always has, and
        // measuring a manifest it would reject is pure cost on every large cold restore.
        if (isForkDaemonProtocol) {
          return deliverInlineSeed()
        }
