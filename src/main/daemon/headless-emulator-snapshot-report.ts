...
import {
  UNREADABLE_ENGINE_MODES,
  combineTerminalModes,
  readEngineModes,
  type EmulatorModeScanners
} from './emulator-terminal-modes'
...
export type EmulatorSnapshotContext = {
  ...
  /** The stream-scanned half of the mode set; the engine half is read here,
   *  under the same poison guard as the serialize it accompanies. */
  modeScanners: EmulatorModeScanners
...
export function buildEmulatorSnapshotReport(ctx: EmulatorSnapshotContext): TerminalSnapshot {
  ...
  const modes = combineTerminalModes(
    ctx.run(
      'modes',
      () => readEngineModes(ctx.term),
      () => UNREADABLE_ENGINE_MODES
    ),
    ctx.modeScanners
  )
