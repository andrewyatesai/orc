...
describe('DaemonPtyAdapter (IPtyProvider)', () => {
  ...
  describe('history integration', () => {
    ...
    it('uploads a large cold-restore seed in bounded protocol chunks', async () => {
      // Why a public daemon: the chunked path is gated on the PUBLIC namespace.
      // The fork's Rust daemon (10xx) implements none of the four transfer RPCs
      // and keeps inline delivery, so pin the protocol that owns the RPCs.
      await server.shutdown()
      server = new DaemonServer({
        socketPath,
        tokenPath,
        protocolVersion: HISTORY_SEED_TRANSFER_PROTOCOL_VERSION,
        spawnSubprocess: (opts) => {
          lastSpawnOpts = opts
          lastSubprocess = createMockSubprocess()
          return lastSubprocess
