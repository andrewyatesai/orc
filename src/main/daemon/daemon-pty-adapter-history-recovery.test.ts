...
// Why the short prefix: the AF_UNIX sun_path cap is 104 bytes on macOS, and the
// fork's 1000+ protocol namespace makes the endpoint name (daemon-v1021.sock)
// two bytes longer than upstream's. Under /var/folders/…/T a longer prefix
// pushes the socket past the cap and every listen() fails with EINVAL.
function createTestDir(): string {
  return mkdtempSync(join(tmpdir(), 'daemon-hist-recovery-'))
}
