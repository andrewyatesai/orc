...
// Why spread the original: transitive importers of child_process (execFile) must still find the
// real members, or they throw at module-init before a single test runs.
...
describe('daemon-init: runRestartDaemon (7-step sequence)', () => {
  ...
  // STA-2376: failed_health_check is the residual "couldn't tell" bucket — the only ranking branch
  // with no attribution behind it, so nothing else would notice if it stopped reporting.
  it('reports the residual failed-health replacement when a cold daemon is actually killed', async () => {
    const mod = await importFresh()
    await mod.initDaemonPtyProvider()
    trackDaemonReplacedMock.mockClear()
    // Unreachable with zero live sessions (the default listSessions mock) and a confirmed kill.
