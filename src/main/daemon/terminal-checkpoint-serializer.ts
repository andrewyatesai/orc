...
/** Newest `rows` CRLF-terminated lines of a serialized scrollback blob. */
function tailScrollbackRows(scrollbackAnsi: string, rows: number): string {
  if (rows <= 0 || scrollbackAnsi.length === 0) {
    return ''
  }
  let start = scrollbackAnsi.length
  for (let kept = 0; kept < rows && start > 0; kept += 1) {
    // The trailing CRLF terminates the line ending at `start`; search before it
    // for the previous line's terminator (−1 → this is the oldest line).
    const previousBreak = start >= 3 ? scrollbackAnsi.lastIndexOf('\r\n', start - 3) : -1
    start = previousBreak === -1 ? 0 : previousBreak + 2
  }
  return scrollbackAnsi.slice(start)
}

/** Why: aterm's snapshot is SPLIT — `scrollbackRows` caps only the history
 *  prepended to snapshotAnsi, while scrollbackAnsi always serializes the whole
 *  history, so the history blob must be trimmed to the same rows or even the
 *  viewport-only floor still carries every scrolled-off row. */
function boundedSnapshotAt(emulator: HeadlessEmulator, rows: number): TerminalSnapshot {
  const candidate = emulator.getSnapshot({ scrollbackRows: rows })
  return { ...candidate, scrollbackAnsi: tailScrollbackRows(candidate.scrollbackAnsi, rows) }
}
...
export async function serializeTerminalCheckpointWithinLimit(
  ...
  try {
    const visibleOnly = boundedSnapshotAt(emulator, 0)
    ...
    while (low <= high) {
      ...
      const candidate = boundedSnapshotAt(emulator, rows)
