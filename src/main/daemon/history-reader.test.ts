...
describe('HistoryReader', () => {
  ...
  describe('cold restore lastCommand (#7596)', () => {
    ...
    it('surfaces the last 633;E command from the replayed log, unescaped', async () => {
      writeIncrementalLog('with-cmd', [
        '$ \x1b]633;E;npm install\x07npm output\r\n',
        '$ \x1b]633;E;echo a\\x3bb\x07a;b\r\n'
      ])

      const info = await reader.detectColdRestore('with-cmd')
      expect(info?.lastCommand).toBe('echo a;b')
    })

    it('reassembles a 633;E split across log records', async () => {
      writeIncrementalLog('split-cmd', ['$ \x1b]633;E;cargo bu', 'ild --release\x07building\r\n'])

      const info = await reader.detectColdRestore('split-cmd')
      expect(info?.lastCommand).toBe('cargo build --release')
    })

    it('leaves lastCommand undefined when the log has no 633;E', async () => {
      writeIncrementalLog('no-cmd', ['plain output, no shell hooks\r\n'])

      const info = await reader.detectColdRestore('no-cmd')
      expect(info).not.toBeNull()
      expect(info?.lastCommand).toBeUndefined()
    })

    it('leaves lastCommand undefined on checkpoint-only restore (no log window)', async () => {
      writeSessionWithCheckpoint(dir, 'cp-only', makeMeta(), makeCheckpoint())

      const info = await reader.detectColdRestore('cp-only')
      expect(info).not.toBeNull()
      expect(info?.lastCommand).toBeUndefined()
    })
  })
