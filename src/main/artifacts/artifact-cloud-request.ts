...
export async function artifactRequest<T>(
  ...
  // net.fetch (Chromium stack), not bare fetch: undici crashes the process on an
  // unread response body (orca#8695), and the 204 path below reads none. It also
  // routes through Electron's configured proxy. See global-fetch-call-site-audit.
  const response = await net.fetch(`${apiUrl}/v1/artifacts${path}`, {
