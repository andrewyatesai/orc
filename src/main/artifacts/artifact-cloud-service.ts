...
import {
  ...
  ARTIFACT_CLOUD_UNCONFIGURED_MESSAGE,
...
export class ArtifactCloudService {
  ...
  private async withAuth<T>(
    ...
    // Fork fail-closed: no first-party artifact host is configured for this build,
    // so surface the coded "not configured" state instead of contacting a vendor.
    if (apiUrl === null) {
      return { status: 'unconfigured', message: ARTIFACT_CLOUD_UNCONFIGURED_MESSAGE }
    }
