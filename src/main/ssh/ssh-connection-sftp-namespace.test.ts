...
// Why: uploadFile stages to a short temp sibling (`<dir>/.orca-tmp-<hex>`) and
// promotes it with an atomic rename, so the staged component never carries the
// destination basename. Match on the staging DIRECTORY to keep the namespace
// mapping pinned while the random id stays free.
const stagedIn = (dir: string): RegExp =>
  new RegExp(`^${dir.replace(/[.+]/g, '\\$&')}/\\.orca-tmp-[0-9a-f]+$`)
...
function createFakeSftp(options?: { pendingRealpath?: boolean; pendingLstat?: boolean }): FakeSftp {
  ...
  const recordRename = (from: string, to: string, cb: (err: Error | null) => void): void => {
    sftp.renames.push([from, to])
    cb(null)
  }
  sftp.rename = vi.fn(recordRename)
  sftp.ext_openssh_rename = vi.fn(recordRename)
  sftp.unlink = vi.fn((path: string, cb: (err: Error | null) => void) => {
    sftp.unlinkPaths.push(path)
    cb(null)
  })
  sftp.end = vi.fn(() => {
    sftp.endCalls += 1
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  it('uploads the bundle into the SFTP namespace directory', async () => {
    ...
    // Staging and the atomic promote both have to land inside the SFTP
    // namespace directory — a mapping leak would show up on either side.
    expect(sftp.writtenPaths).toEqual([
      expect.stringMatching(stagedIn(`${SFTP_HOME}/${RELAY_DIR}`))
    ])
    expect(sftp.renames).toEqual([[sftp.writtenPaths[0], `${SFTP_HOME}/${RELAY_DIR}/relay.js`]])
    expect(sftp.endCalls).toBe(1)
  })
