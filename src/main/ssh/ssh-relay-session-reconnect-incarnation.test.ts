...
describe('SshRelaySession reconnect incarnation ordering', () => {
  ...
  it('keeps the attached PTY when incarnation backfill persistence fails', async () => {
    ...
    // Reattach marks leases in memory only ({ flush: false }) and collapses the
    // per-PTY disk writes into a single trailing flush.
    expect(mockStore.markSshRemotePtyLease).toHaveBeenCalledWith(
      'target-1',
      'pty-live',
      'attached',
    ...
    expect(vi.mocked(mockStore.markSshRemotePtyLease).mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(mockStore.flush).mock.invocationCallOrder[0]!
    )
