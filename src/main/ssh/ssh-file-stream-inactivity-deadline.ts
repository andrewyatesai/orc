...
// Why: a remote file stream can stall indefinitely if the relay hangs mid-transfer;
// bound uninterrupted inactivity while treating host sleep as a pause, not a stall.
export function createSshFileStreamInactivityDeadline(
  ...
  const arm = (): void => {
    ...
    // Why: never keep the event loop alive on the deadline alone.
    timer.unref?.()
  }
  const reset = (): void => {
