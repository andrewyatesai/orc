...
function installSftpHandlers(sftp: SFTPWrapper, backingRoot: string, operations: string[]): void {
  ...
  sftp.on('OPEN', (requestId, remotePath, flags) => {
    ...
    // Honor EXCL so the staged temp's no-clobber contract is exercised on the wire.
    void open(localPath, flags & SFTP_OPEN_EXCL ? 'wx' : 'w').then(
      (file) => {
        const id = nextFileId++
        files.set(id, file)
  ...
  sftp.on('REMOVE', (requestId, remotePath) => {
    operations.push(`REMOVE:${remotePath}`)
    const localPath = backingPath(backingRoot, remotePath)
    if (!localPath) {
      sftp.status(requestId, SFTP_STATUS_NO_SUCH_FILE)
      return
    }
    void rm(localPath).then(
      () => sftp.status(requestId, SFTP_STATUS_OK),
      (error: unknown) => sendFsError(sftp, requestId, error)
    )
  })
  // SFTP v3 RENAME is no-clobber: it fails when the destination already exists.
  sftp.on('RENAME', (requestId, oldPath, newPath) => {
    operations.push(`RENAME:${oldPath}->${newPath}`)
    const localFrom = backingPath(backingRoot, oldPath)
    const localTo = backingPath(backingRoot, newPath)
    if (!localFrom || !localTo) {
      sftp.status(requestId, SFTP_STATUS_NO_SUCH_FILE)
      return
    }
    void lstat(localTo).then(
      () => sftp.status(requestId, SFTP_STATUS_FAILURE),
      () =>
        rename(localFrom, localTo).then(
          () => sftp.status(requestId, SFTP_STATUS_OK),
          (error: unknown) => sendFsError(sftp, requestId, error)
        )
    )
...
it('uploads through a verified split namespace over a real ssh2 SFTP session', async () => {
  ...
  try {
    await withSplitNamespaceFixture(async ({ connection, fixture, backingRelayDir }) => {
      ...
      // The upload stages to a temp sibling inside the mapped directory and is
      // promoted by RENAME, so the destination never holds a partial file.
      const stagedOpen = fixture.operations.find((entry) =>
        entry.startsWith(`OPEN:${SFTP_RELAY_DIR}/nested/.orca-tmp-`)
      )
      expect(stagedOpen).toBeDefined()
      expect(fixture.operations).toContain(
        `RENAME:${stagedOpen!.slice('OPEN:'.length)}->${SFTP_RELAY_DIR}/nested/payload.bin`
      )
      expect(fixture.operations).not.toContain(`OPEN:${SFTP_RELAY_DIR}/nested/payload.bin`)
    })
  } finally {
