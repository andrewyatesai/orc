...
async function installNativeDepsWithoutNodePty(
  ...
  // Why --ignore-scripts with no `npm rebuild`: this host provably has no compiler, so gyp would only
  // fail; @parcel/watcher resolves its prebuilt at require time, and the probe below reports if it can't.
  await execHostCommand(
    conn,
    hostPlatform,
    commandWithNodePath(
