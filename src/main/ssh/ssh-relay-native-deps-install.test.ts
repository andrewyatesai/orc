...
describe('installNativeDeps (via deployAndLaunchRelay)', () => {
  ...
  it('connects without node-pty when the remote toolchain is missing, instead of failing the host', async () => {
    ...
    // (The scoped rebuild rides inside the install command, so only a standalone one counts.)
    expect(execCalls.some((c) => c.includes('npm rebuild') && !c.includes('npm install'))).toBe(
      false
    )
