...
// Why: the provider epoch fences a specific transport incarnation; the generation (shared with staged
// mutations) fences the mutation window. Together they form the authority the renderer retries under.
...
export function rotateSshProviderAuthority(targetId: string): DirectSshAuthority {
  ...
  if (nextGeneration !== previousGeneration + 1) {
    // Why: a scope roll revoked every target's generation, so every prior authority is void.
