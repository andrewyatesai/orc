...
// Why: Windows env blocks spell the search path `Path`; the fork otherwise reads only `PATH`, so honor whichever
// case-insensitive spelling is present (falling back to the host env, then `Path`) before splitting it.
function resolvePathEnvKey(
  ...
  return firstPathEnvKey(env) ?? firstPathEnvKey(process.env) ?? 'Path'
}

function firstPathEnvKey(env: Record<string, string | undefined>): string | undefined {
