...
describe('destructive teardown when a PTY stop cannot be proven', () => {
  ...
  it('removes the reported wedged automation workspace without --force', async () => {
    const worktreeId = 'repo-1::C:/Workspaces/admin/orca/workspaces/repo/auto-review-run-28'
  ...
  // exists; pin it before it can silently vanish under a refactor.
  ...
  // Why: "we could not ask" must read differently from "it is still live". Fork
  // has no provider-inventory toggle, so drive a clean first list (provider sweep)
  // and a failing second list (verification) through a stale registry row.
  ...
  it('warns with the specific sweep failure, not the generic deadline', async () => {
    ...
    try {
      ...
      expect(warning.indexOf('ssh channel closed')).toBeLessThan(warning.indexOf('Timed out waiting'))
    } finally {
      vi.useRealTimers()
      warn.mockRestore()
    }
