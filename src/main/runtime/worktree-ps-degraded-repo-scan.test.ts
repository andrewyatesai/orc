...
const LOCAL_REPO_PATH = '/home/me/dev/app'
...
describe('worktree.ps on a degraded repo scan', () => {
  ...
  // Why: a healthy scan reporting zero worktrees while the repo still owns lineage is almost always a degraded
  // observation (a repo always has its main worktree), so the guard refuses to wipe lineage on one empty verdict.
  it('does not prune lineage when a healthy scan reports zero worktrees but lineage remains', async () => {
    const removeWorktreeLineage = vi.fn()
    const removeWorkspaceLineage = vi.fn()
    listWorktreesStrictMock.mockResolvedValue([])
    const runtime = new OrcaRuntimeService(
