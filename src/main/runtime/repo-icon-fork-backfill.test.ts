...
// The GitHub-avatar builder is the Rust repo-icon core (napi); mock only that
// leaf seam with the same URL/label it emits, so the real backfill +
// detectGitHubAvatarIcon + githubAvatarSlug selection logic runs unchanged.
vi.mock('../rust-repo-icon', () => ({
  githubAvatarIcon: (slug: { owner: string; repo: string }) => ({
    type: 'image',
    src: `https://github.com/${encodeURIComponent(slug.owner)}.png?size=64`,
    source: 'github',
    label: `${slug.owner}/${slug.repo}`
  }),
  faviconUrlFromWebsite: () => null
}))
