...
export class OrchestrationDb {
  ...
  constructor(dbPath: string | ':memory:') {
    ...
    this.createTables()
    this.migrate()
    // After migrate(): a pre-v9 upgrade rebuilds the messages table, which would
    // drop a column added before it.
    this.createForkPaneIdentityColumnsIfMissing()
  ...
  // else. Repaired on every open instead, which also reaches an already-at-head DB
  // that migrate() short-circuits.
  private createForkPaneIdentityColumnsIfMissing(): void {
    ...
    if (!this.hasColumn('messages', 'recipient_pane_key')) {
      this.db.exec('ALTER TABLE messages ADD COLUMN recipient_pane_key TEXT')
    }
    if (!this.hasColumn('decision_gates', 'origin_message_id')) {
      this.db.exec('ALTER TABLE decision_gates ADD COLUMN origin_message_id TEXT')
    }
  }
  ...
  insertMessage(msg: {
    ...
    // Why: the remint-stable recipient identity, persisted so delivery can follow the
    // pane after the addressed handle goes stale (#9163).
  ...
  setDispatchTimestamps(
    ...
    this.db
      ...
      .run(dispatchedAt ?? null, lastHeartbeatAt ?? null, dispatchId)
  }
  ...
  createGate(gate: {
    ...
    this.db
      .prepare(
        'INSERT INTO decision_gates (id, run_id, task_id, question, options, origin_message_id) VALUES (?, ?, ?, ?, ?, ?)'
      )
      .run(
        id,
  ...
  // Why: several orchestrators may run concurrently in one workspace (#4389), so
  // lifecycle gating must see all of them, not only the newest.
  getActiveCoordinatorRuns(): CoordinatorRun[] {
    return this.db
      .prepare(
        "SELECT * FROM coordinator_runs WHERE status = 'running' ORDER BY created_at DESC, rowid DESC"
      )
