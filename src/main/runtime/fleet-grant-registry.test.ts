...
import { FLEET_GRANT_ENV_VAR, containsFleetGrantEnv, stripFleetGrantEnv } from '../../shared/fleet-grant'
...
describe('FleetGrantRegistry', () => {
  it('allows the op it granted on the pane it granted', () => {
    ...
    const grant = registry.issue({ runId: 'run_1', generation: 0, ops: ['write'], targets: [target] })
    expect(
  ...
  it('refuses a caller presenting nothing — the R0 default', () => {
    const { registry } = makeRegistry()
    expect(registry.check(null, { op: 'write', handle: 'worker-1', incarnationId: 'inc_1' })).toEqual({
      allowed: false,
  ...
  it('refuses an unknown secret', () => {
    const { registry } = makeRegistry()
    expect(registry.check('made-up', { op: 'write', handle: 'worker-1', incarnationId: 'inc_1' })).toEqual({
      allowed: false,
  ...
  it('refuses an op the grant does not carry', () => {
    const { registry } = makeRegistry()
    const grant = registry.issue({ runId: 'run_1', generation: 0, ops: ['read'], targets: [target] })
    expect(
      registry.check(grant.secret, { op: 'write', handle: 'worker-1', incarnationId: 'inc_1' })
  ...
  it('revoking one run leaves another alone', () => {
    const { registry } = makeRegistry()
    const mine = registry.issue({ runId: 'run_1', generation: 0, ops: ['write'], targets: [target] })
    const other = registry.issue({ runId: 'run_2', generation: 0, ops: ['write'], targets: [target] })
