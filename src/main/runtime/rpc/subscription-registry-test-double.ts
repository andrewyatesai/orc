 ...
 * subscribe/teardown ownership must use this instead of a bare Map.
 */
...
/**
 * Teaches a hand-built runtime double the owned-subscription API that
 * `terminal.subscribe`/`terminal.unsubscribe` now call, by routing it through
 * whatever `registerSubscriptionCleanup`/`cleanupSubscription` the double already
 * carries. This reproduces the pre-STA-4510 routing (a keyed teardown), so existing
 * routing tests keep their exact behavior; the *ownership* semantics are pinned by
 * the runtime-level tests in `orca-runtime.test.ts` and by
 * `createSubscriptionRegistryDouble`, not here.
 */
export function attachOwnedSubscriptionCleanupRouting(runtime: OrcaRuntimeService): OrcaRuntimeService {
  const target = runtime as unknown as OwnedSubscriptionRouting
  if (!target.registerOwnedSubscriptionCleanup) {
    target.registerOwnedSubscriptionCleanup = (id, cleanup, connectionId) => {
      target.registerSubscriptionCleanup?.(id, cleanup, connectionId)
      return { releaseIfCurrent: () => target.cleanupSubscription?.(id) }
    }
  }
  if (!target.cleanupSubscriptionIfOwnedByConnection) {
    target.cleanupSubscriptionIfOwnedByConnection = (id) => {
      target.cleanupSubscription?.(id)
      return true
    }
  }
  return runtime
