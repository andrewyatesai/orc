...
async function serializeBudgetedRequestedSnapshot(
  ...
  // Why authoritative: a requested snapshot must prefer the provider's own buffer, retiring it on timeout.
  // Why scrollbackRows:0: scrollbackAnsi is the sole history source, so prepending history into
  // snapshotAnsi too would replay the recent rows twice; history is bounded below on a line boundary.
  // Fed §2.4 consequence: a provider-sourced buffer (daemon) carries no anchor inputs, so this payload
  // gets no hostRowAnchor — never mint one from the headless mirror, whose absolute rows are a DIFFERENT
  // coordinate space; the client degrades to terminal.searchContext instead of jumping to a wrong row.
