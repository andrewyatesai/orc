...
describe('orchestration RPC methods', () => {
  ...
  it('registers all expected methods', () => {
    ...
    // 34 upstream methods plus this fork's orchestration.runLog.
    ...
    expect(registry.has('orchestration.runLog')).toBe(true)
    ...
    // Why pinned: without a reader the coordinator's stale-heartbeat warning — the only
    // hang detector in the codebase — is generated and thrown away.
    expect(registry.has('orchestration.runLog')).toBe(true)
  ...
  describe('orchestration.send', () => {
    ...
    it('ignores caller-supplied pane claims and uses the runtime-observed pane', async () => {
      ...
      // pane key rides along on the notify (delivery-follows-identity, #9163).
    ...
    it('still wakes waiters for a heartbeat on an active dispatch', async () => {
      ...
      expect(notify).toHaveBeenCalledWith(`run:${activeRunId}`, 'heartbeat', undefined)
    })
    ...
    // Why: these cases assert the real handle->paneKey memory, which setup()'s
    // coordinator-only pane-key stub would answer for.
    function setupWithRealPaneKeys(): void {
      setup()
    ...
    it('delivers a message addressed to a stale handle into the current pane', async () => {
      vi.useFakeTimers()
      try {
        setupWithRealPaneKeys()
        ...
        // Why: send no longer pushes (delivery is ledger-driven), so drive the
        // delivery seam directly — the row's pane key must still reach pty-2.
        runtime.deliverPendingMessagesForHandle(stale, REMINT_PANE_KEY)
