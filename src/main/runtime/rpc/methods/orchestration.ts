    ...
    handler: async (
      ...
      if (!isGroupAddress(to)) {
        ...
        const recipientPaneKey =
          runtime.getTerminalPaneKey(to) ?? lastPersistedRecipientPaneKey(db, to)
        ...
        if (msg.type === 'worker_done' || msg.type === 'heartbeat') {
          ...
          if (reconciled.action === 'rejected') {
            ...
            runtime.notifyMessageArrived(to, rejection.type, recipientPaneKey)
            return { message: rejection, lifecycle: reconciled }
          }
        }
        runtime.notifyMessageArrived(to, msg.type, recipientPaneKey)
        return { message: msg }
      }
