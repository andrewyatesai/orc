...
export class E2EEChannel {
  ...
  private dispatchV1Text(plaintext: string): void {
    // Why: streaming emits (e.g. terminal.subscribe) can outlive destroy(); with a
    // cleared sharedKey tweetnacl throws "unexpected type, use Uint8Array" from
    // nacl.box.after, so guard both socket state AND the key.
