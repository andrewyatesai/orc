...
// Parser cost is measured on aterm (the fork's headless engine) rather than
// headless xterm — aterm is what actually parses multiplexed viewer output here.
async function measureHeadlessEngineParsing(viewers: number): Promise<ParserMeasurement> {
  ...
  const terminals = Array.from(
    { length: viewers },
    () => new HeadlessEmulator({ cols: 120, rows: 40, scrollback: 5_000 })
  )
  ...
  return {
    ...
    // Note: aterm holds grid/scrollback in Rust, so this counts only the JS-side
    // retention of the same workload, not the engine's total footprint.
...
describe.skipIf(!benchEnabled)('terminal multiplex parsed-credit benchmark', () => {
  it('reports RTT, fairness, protocol allocations, and measured aterm parser cost', async () => {
    const parserMeasurements = new Map<number, ParserMeasurement>()
    for (const viewers of [1, 4, 8]) {
      parserMeasurements.set(viewers, await measureHeadlessEngineParsing(viewers))
    }
    const rows = [1, 20, 100].flatMap((rttMs) =>
      [1, 4, 8].map((viewers) => {
        ...
        return {
          ...
          engineWriteAllocations: parser.engineWrites
        }
      })
    )
