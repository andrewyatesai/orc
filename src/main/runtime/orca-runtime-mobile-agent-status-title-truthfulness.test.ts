...
//
// Fork note: the fork tracks OSC-title observations by sequence number, not a
// wall-clock epoch, so the identity-only fallback dates its `done` by the hook
// session row's receipt time (getHookAgentRowForPane.providerSessionReceivedAt),
// the only cross-machine-stable evidence stamp available here.
...
/** Publish a renderer graph where the pane has been released: the renderer
 *  omits `launchAgent` entirely, so the host must not resurrect it from PTY
 *  launch provenance. */
...
describe('mobile session tabs: identity-only fallback freshness', () => {
  ...
  it('dates the identity-only done by its hook evidence, not by output bytes', async () => {
    ...
    expect(await projectAgentStatus(runtime)).toEqual(expect.objectContaining({ updatedAt: T0 }))
  })
})
