...
describe('getRuntimeFolderWorkspaceRootId', () => {
  it.each([
    { label: 'posix path', input: repo(), expected: 'repo-1::/home/dev/projects/site' },
    {
      label: 'windows path',
      input: repo({ path: 'C:\\repos\\dev\\site' }),
      expected: 'repo-1::C:\\repos\\dev\\site'
    },
    {
...
describe('getRuntimeFolderWorkspaceInstanceId', () => {
  it('appends the instance id after the separator', () => {
    expect(getRuntimeFolderWorkspaceInstanceId(repo(), 'abc')).toBe(
      `repo-1::/home/dev/projects/site${FOLDER_WORKSPACE_INSTANCE_SEPARATOR}abc`
    )
  })

  it('appends an empty instance id without complaint', () => {
    expect(getRuntimeFolderWorkspaceInstanceId(repo(), '')).toBe(
      `repo-1::/home/dev/projects/site${FOLDER_WORKSPACE_INSTANCE_SEPARATOR}`
    )
  })
})

describe('isRuntimeFolderWorkspaceIdForRepo', () => {
  const root = 'repo-1::/home/dev/projects/site'
  ...
  it.each([
    ...
    { label: 'another repo', worktreeId: 'repo-2::/home/dev/projects/site', expected: false },
...
describe('mergeRuntimeFolderWorkspace', () => {
  it('projects repo + meta onto a branchless, headless worktree', () => {
    const merged = mergeRuntimeFolderWorkspace(repo(), 'repo-1::/home/dev/projects/site', meta())

    expect(merged).toEqual({
      id: 'repo-1::/home/dev/projects/site',
      repoId: 'repo-1',
      path: '/home/dev/projects/site',
      head: '',
      branch: '',
  ...
  it('marks a non-root id as a secondary workspace', () => {
    const worktreeId = `repo-1::/home/dev/projects/site${FOLDER_WORKSPACE_INSTANCE_SEPARATOR}abc`
    ...
    expect(merged.path).toBe('/home/dev/projects/site')
  })
