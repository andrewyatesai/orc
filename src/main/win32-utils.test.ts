...
describe('getSpawnArgsForWindows', () => {
  ...
  it('ignores detachedGui on non-win32 and passes the command through unchanged', () => {
    withPlatform('darwin', () => {
      const { spawnCmd, spawnArgs } = getSpawnArgsForWindows(
        '/Applications/IntelliJ.app/bin/idea',
        ['/tmp/workspace'],
        { detachedGui: true }
      )
      expect(spawnCmd).toBe('/Applications/IntelliJ.app/bin/idea')
      expect(spawnArgs).toEqual(['/tmp/workspace'])
    })
  })
