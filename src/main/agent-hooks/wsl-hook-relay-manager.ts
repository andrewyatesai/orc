...
export class WslHookRelayManager {
  ...
  /** Wires the live agent-status-hooks resolver once the store exists (the singleton
   *  is built before it), so the manager gate reads the current toggle, not null. */
