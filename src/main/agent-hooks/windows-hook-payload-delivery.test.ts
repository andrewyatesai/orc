// Why: the Windows managed launcher must hand the hook the JSON Claude Code pipes in. A
// conhost/ConPTY host (`conhost.exe --headless … /d /c <script>`) is not a no-window wrapper:
// it re-hosts the child on a pseudoconsole, so the piped payload never reaches the script and
// the curl POST carries nothing. The fork runs an encoded hidden-PowerShell launcher precisely
// to keep stdin, but shape, exit-code and stdout assertions all stay green under either launcher
// because none of them ever pipe a payload. This suite pipes one and follows it to the listener,
// so a launcher that takes stdin away from the hook fails here (#14818).
