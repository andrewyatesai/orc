...
// Early rollouts stored response items unwrapped (no `payload` envelope).
...
// The canonical paginated format wraps each turn item in an `item_completed`
// event; the visible turn item carries the safe user/assistant content.
...
// Turn-item content mixes text/image variants across format generations; only
// known-safe kinds become blocks, so model-only fields are never surfaced.
