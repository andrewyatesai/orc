...
export function enrichMissingRepoGitRemoteIdentities(
  ...
  if (sweepInFlight) {
    // Why: the sweep probes candidates in bounded fan-out, so one fresh pass per list IPC still
    // piles up behind a slow probe — queue a single follow-up instead, which still picks up repos
    // added mid-pass.
    rerunRequested = true
    return
  }
