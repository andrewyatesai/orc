...
// Why: the fork gates the install on a real instance scan + on-disk handoff marker; stub both
// so this suite still reaches the installer handoff that raises the recovery error.
vi.mock('./updater-conflicting-app-instances', () => ({
  findConflictingAppInstancePids: vi.fn(async () => [] as number[])
}))
vi.mock('./startup/update-install-launch-gate', () => ({
  writeUpdateInstallHandoffMarker: vi.fn(),
  clearUpdateInstallHandoffMarker: vi.fn()
}))
// Why: the fork self-manages the check on every platform, but this suite drives the native
// updater's error path — pin the policy to 'automatic' so those handlers are registered.
