...
async function performQuitAndInstall(): Promise<void> {
  ...
  try {
    await withUpdaterSpan({ stage: 'install' }, async (span) => {
      ...
      // Why: run after the conflict gate so a blocked install never leaves a
      // supervised serve handoff request behind for the supervisor to act on.
      if (
...
export function getRemoteServerUpdateSupport(): RemoteServerUpdateSupport {
  ...
  if (!autoUpdaterInitialized || getUpdateInstallMode() === 'manual') {
    // Why: manual-release platforms mark the updater initialized but never wire
    // the native updater, so a remote caller must not be told installs are automatic.
    return {
      installMode: updateInstallMode,
      automatic: false,
...
async function runManualReleaseCheck(
  ...
  backgroundCheckPromotedToUserInitiated = false
  userInitiatedCheck = false
  // Why: lastGoodTag only rides the 'not-ready' variant of the readiness union.
  const lastGoodTag = result.state === 'not-ready' ? result.lastGoodTag : undefined
  const releaseTag = result.tags[0] ?? lastGoodTag ?? null
  if (releaseTag) {
...
function runBackgroundUpdateCheck(
  ...
  if (usesSelfManagedCheck(getUpdateInstallMode())) {
    // Why: the self-managed check IS launched here, so report it as launched or the timer re-arms over it.
...
export function checkForUpdatesFromMenu(options?: UpdateCheckOptions): void {
  ...
  // Why: a local build feed never leaves the machine, so it runs before the
  // dormant-feed gate.
...
export function quitAndInstall(): void {
  ...
  // Why: an unsupervised `orca serve` refuses the install outright, so it must gate
  // ahead of the swap — swapping and relaunching here would kill the serve session,
  // and ahead of the retry telemetry, which would otherwise record a deferred install.
