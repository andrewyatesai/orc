...
function buildDetectedGitWorktrees(
  ...
  // Hoisted out of the map: the classifier memoizes its scratch matcher on this
  // array's identity, so one array per fan-out normalizes each checkout once.
  const agentScratchCheckoutPaths = [repo.path, ...liveWorktrees.map((worktree) => worktree.path)]
...
function buildDisconnectedDetectedWorktrees(
  ...
  const agentScratchCheckoutPaths = [repo.path, ...worktrees.map((worktree) => worktree.path)]
...
export function registerWorktreeHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event, args: RemoveWorktreeArgs): Promise<RemoveWorktreeResult> => {
      ...
      // Why: scope preserved-branch cleanup by host so the same worktreeId on two hosts keeps distinct routes.
