...
import { startGitCommonPrimaryMetadataPoll } from './worktree-git-common-primary-metadata-poll'
...
async function startGitCommonNarrowWatch(
  ...
  // Why: once the crash fuse latches (or the child is already gone), the watcher
  // process will never come back this session — re-arming the existence poll
  // just retries a dead subscribe forever. Fall back to structural polling so
  // worktree changes are still detected, and never re-upgrade to a narrow watch.
...
export async function startGitCommonWatch(
  ...
  if (platform === 'darwin') {
    const [narrowWatch, primaryMetadataPoll] = await Promise.all([
      startGitCommonNarrowWatch(target, onEvents, pollIntervalMs, visibility, onFullScan),
      startGitCommonPrimaryMetadataPoll(target.path, onEvents, pollIntervalMs, visibility, onFullScan)
    ])
    return {
      unsubscribe: async () => {
