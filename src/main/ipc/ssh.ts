...
// Why: the generation counter lives in ssh-connection-generation so staged
// mutations (filesystem, runtime, tab close) validate against the same token.
...
export function registerSshHandlers(
  ...
  async function connectTarget(targetId: string): Promise<SshConnectionState> {
    ...
    pendingTransportReconnects.delete(targetId)
    // Why: doConnect advances the generation synchronously before its first
    // await (and skips it on the already-live refresh path), so read it back.
