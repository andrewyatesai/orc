...
export async function removeRegisteredSshTarget(targetId: string): Promise<void> {
  ...
  await runTargetLifecycle(targetId, async () => {
    try {
      // Why: removal is destructive — dispose (not detach) so the relay shuts down and remote PTY
      // leases terminate rather than being preserved for a reattach to a deleted target.
...
export function registerSshHandlers(
  ...
  async function connectTarget(targetId: string): Promise<SshConnectionState> {
    ...
    pendingTransportReconnects.delete(targetId)
    // Why: doConnect rotates the provider authority synchronously before its first await
    // (and skips it on the already-live refresh path), so read the token back after.
