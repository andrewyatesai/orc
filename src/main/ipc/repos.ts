...
function alignRepoWithRequestedProject(
  ...
  if (setup.projectId !== projectId) {
    ...
    // Why: the selected project can exist only on the source host, so its structured identity
    // travels with the request; it is only trusted when it resolves to the requested project.
    const requestedIdentity =
      requestedProviderIdentity &&
      getProjectIdForProviderIdentity(requestedProviderIdentity) === projectId
        ? requestedProviderIdentity
        : undefined
    const identityStamp =
      getProjectIdentityRepoStamp(project) ??
      (requestedIdentity
        ? { upstream: { owner: requestedIdentity.owner, repo: requestedIdentity.repo } }
        : null)
    if (!identityStamp) {
    ...
    const providerHost = (project?.providerIdentity ?? requestedIdentity)?.host
    const stamp =
