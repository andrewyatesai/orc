...
// Why: the launch-command detectors live in the Rust orca-text core (napi here, wasm on the relay).
import { detectExplicitPiAgentKindFromCommand } from '../pi/rust-pi-agent-kind'
...
export function registerPtyHandlers(
  ...
  runtime?.setPtyController({
    spawn: async (args) => {
      ...
      // Why claudePin.command, not args.command: the Claude session-id pin and the
      // Codex resume-argv drop each rewrite the launch command, and only one command
      // reaches spawnOptions — chain them so neither rewrite is lost (the two are
      // mutually exclusive, so this is a pass-through on both agents' own paths).
      const codexResumeLaunch = codexResumePreparation
        ? await resolveCodexResumeLaunch(claudePin.command, codexResumePreparation)
        : noCodexResumeLaunch(claudePin.command)
