...
export async function createLocalWorktree(
  ...
  notifyWorktreesChanged(mainWindow, repo.id)
  // Why: create can surface three independent warnings — `.worktreeinclude` copy skips, startup/setup,
  // and branch publish; the first two read as one sentence, publish stays on its own line.
