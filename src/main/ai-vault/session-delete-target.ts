...
export function validateAiVaultSessionDeleteTarget(
  ...
  const source = AI_VAULT_DELETE_SOURCES[agent]
  const roots = source
  ...
  if (!isDiscoverableDeleteTarget(source.extensions, source.filePredicate, resolvedPath)) {
...
// Mirrors walkSessionFiles' file-accept rule (extension + optional predicate).
function isDiscoverableDeleteTarget(
  extensions: readonly string[],
  filePredicate: ((resolvedPath: string) => boolean) | undefined,
  resolvedPath: string
): boolean {
  if (!extensions.includes(extname(resolvedPath).toLowerCase())) {
    return false
  }
  return filePredicate?.(resolvedPath) ?? true
}
