...
async function removeOne(
  ...
  // WSL removes inside the distro: 9P stat is unreliable and the WSL virtual
  // volume has no Recycle Bin for trashItem. Returns false for a non-WSL path,
  // so the local-fs guards below run for every ordinary path. The validator
  // already confirmed this path is textually inside the agent's WSL roots.
  if (
