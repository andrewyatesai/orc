...
export function deleteWorktreeHistoryDir(worktreeId: string): void {
  ...
  // Why: daemon scrollback is keyed by `${worktreeId}@@…` session ids, not the hashed dirs below, so
  // it survives worktree removal unless swept here. Bounded per worktree, so it stays off the queue.
  try {
    sweepWorktreeDaemonSessionHistory(historyRoot, worktreeId)
  } catch (err) {
    console.warn(
