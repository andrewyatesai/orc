...
export async function listMcpIssues(
  ...
  const { entries, failures: entryFailures } = await getIssueListEntries(request.workspaceId)
  if (entries.length === 0) {
    if (entryFailures[0]) {
...
// Async because the fork loads the Linear SDK lazily (getClients/getFanoutClientEntries).
async function getIssueListEntries(workspaceId?: string | 'all'): Promise<{
  entries: LinearClientForWorkspace[]
  failures: WorkspaceReadFailure[]
}> {
  if (workspaceId === 'all') {
    return await getFanoutClientEntries()
  }
  if (workspaceId) {
