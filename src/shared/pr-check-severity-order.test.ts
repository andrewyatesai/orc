...
// The GitLab status mappers are rust-sole (orca-core gitlab_pipeline_checks), so reach the real
// mapper through the shared dispatch seam rather than a deleted TS copy.
const gitLabConclusion = (status: string, allowFailure: boolean): PRCheckDetail['conclusion'] =>
  requireOrcaDispatch('gitlab-pipeline-checks', 'mapGitLabPipelineJobStatusToConclusion', {
    status,
    allowFailure
...
describe('PR check severity order', () => {
  ...
  it('orders normalized provider states with stable ties', () => {
    const checks = [
      ...
      check('manual', gitLabConclusion('manual', true)),
      check('pass-a', 'success'),
      check('pass-b', 'success')
    ]

    // An optional manual GitLab gate is neutral, so it sinks below the passing checks a reviewer
    // reads first; a blocking one is action_required here, so it sorts with the failures.
    ...
    expect(getCheckSeverityRank(gitLabConclusion('manual', false))).toBe(
      getCheckSeverityRank('failure')
    )
  })
})
