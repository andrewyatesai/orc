...
/**
 * Real chunk boundaries captured from a fish prompt-accept over node-pty: arm 2031 while
 * painting the prompt, echo the typed line, withdraw 2031, exec the command. The measured
 * gap between the arm and the withdraw is shorter than a renderer IPC hop, so a reply queued
 * on the subscribe cannot reach fish — only the child that replaced it (#9993).
 */
...
describe('a fish prompt-accept burst is tracked, never answered (#9993)', () => {
  ...
  it('contains no color-scheme query, so silence is the whole correct answer', () => {
    ...
    // fish never sends the protocol's query (`CSI ?996n`). It only ever subscribes, so there
    // is nothing here that a `CSI ?997;Nn` could be a reply to.
