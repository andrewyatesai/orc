export class RelayFrameBuffer {
  ...
  /** Copy the residue out of the retained subarray views, so the caller's chunk can be GC'd and can no longer mutate under us. */
  compact(): void {
    this.chunks = this.bytes > 0 ? [Buffer.concat(this.chunks, this.bytes)] : []
  }
