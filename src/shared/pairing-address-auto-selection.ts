...
// Why: container/VM bridges are host-local — a phone can never reach docker0 or vmnet8 — but they
// enumerate as ordinary non-internal IPv4, so auto-advertising one makes the direct path silently
// lose the pairing race. Keyed on interface name, not subnet: Docker's 172.16/12 pool overlaps real
...
// address", which Relay tolerates (it carries its own invite) and the LAN-only path refuses. The
// tailnet classifier is injected because the fork's impl is Rust-backed and side-specific (napi in
// main, wasm in renderer), so there is no single module both sides could import.
export function selectAutoAdvertisedPairingAddress(
  interfaces: readonly PairingNetworkInterface[],
  isTailnetIPv4Address: (address: string) => boolean
): string | undefined {
