...
export function createSequencedSetupAgentCommands(args: {
  ...
  // Why: the shell recorded with the runner wins because it knows the actual executable
  // (wsl.exe / Git Bash); the pane's configured family is the fallback for runners written
  // before that was recorded, so Git Bash still gets POSIX delivery, not cmd.exe (#6896).
  const resolution = resolveSetupRunnerCommand(
    args.runnerScriptPath,
    args.platform,
    args.shell ?? args.terminalShellFamily
  )
...
// Why: base64 removes every cmd/PowerShell quoting hazard the batch wrapper had to escape (#8787).
