...
describe('symbolic full-name resolution', () => {
  ...
  it('categorizes a branch when git echoes the option marker', async () => {
    ...
    const result = await loadGitHistoryFromExecutor(executor, '/repo', { baseRef: 'feature' }, testParseLog)
  ...
  it('categorizes a tag when git echoes the option marker', async () => {
    ...
    const result = await loadGitHistoryFromExecutor(executor, '/repo', { baseRef: 'v1.0.0' }, testParseLog)
  ...
  it('falls back to the commits category for a bare revision', async () => {
    ...
    const result = await loadGitHistoryFromExecutor(executor, '/repo', { baseRef: 'some-rev' }, testParseLog)
