...
import { COMMIT_MESSAGE_AGENT_SPECS, getCommitMessageAgentSpec } from './commit-message-agent-spec'
import type { TuiAgent } from './types'

const commitMessageAgentIds = Object.keys(COMMIT_MESSAGE_AGENT_SPECS) as TuiAgent[]
...
describe('getAgentModelProbeSpec', () => {
  ...
  it('aliases commit-message agents by identity rather than copying them', () => {
    ...
    // `modelDiscovery.parse` and break existing discovery.
    for (const id of commitMessageAgentIds) {
  ...
  it('keeps grok out of the commit-message registry', () => {
    ...
    expect(commitMessageAgentIds).not.toContain('grok')
  })
