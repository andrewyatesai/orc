...
import { TUI_AGENT_CONFIG, type DraftPasteReadySignal } from './tui-agent-config'
import {
...
describe('grok production wiring reaches the composer-glyph signal', () => {
  // Why: the trace-replay above hardcodes 'grok-composer-prompt'. This proves the
  // shipped config actually resolves grok to that signal, exactly as the scanner
  // callers (orca-runtime's paste waiter and the renderer paste path) build it via
  // TUI_AGENT_CONFIG[agent].draftPasteReadySignal — so the fast path is reachable.
  it('routes grok launch drafts through grok-composer-prompt with no native draft plan', () => {
    expect(TUI_AGENT_CONFIG.grok.draftPasteReadySignal).toBe('grok-composer-prompt')
    // No draft prompt flag/env means grok falls through to paste-after-ready,
    // where this signal applies, instead of a native prefill launch.
    expect(TUI_AGENT_CONFIG.grok.draftPromptFlag).toBeUndefined()
    expect(TUI_AGENT_CONFIG.grok.draftPromptEnvVar).toBeUndefined()
  })

  it('delivers on the composer frame when the scanner is built from the resolved config', () => {
    const signal =
      TUI_AGENT_CONFIG.grok.draftPasteReadySignal ?? 'render-quiet-after-bracketed-paste'
    const readyAt = replayReadyAtMs(signal, GROK_STARTUP_PTY_TRACE)
    expect(readyAt).not.toBeNull()
    expect(readyAt).toBeLessThan(1000)
  })
})
