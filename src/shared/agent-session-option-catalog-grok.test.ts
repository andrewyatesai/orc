...
describe('grok session option catalog', () => {
  ...
  it('treats a successful discovery as authoritative, unlike the other agents', () => {
    ...
    expect(GROK_SESSION_OPTION_CATALOG.defaultModelIsCliDefault).toBe(true)
    for (const agent of ['claude', 'codex', 'gemini', 'cursor'] as const) {
      ...
      expect(getAgentSessionOptionCatalog(agent)?.defaultModelIsCliDefault).toBeUndefined()
    }
  })

  it('parses the live listing through the catalog listModels hook', () => {
    const parsed = GROK_SESSION_OPTION_CATALOG.listModels!.parse(
      'Available models:\n  * grok-4.5 (default)\n  * grok-build\n'
    )
...
describe('grok launch args', () => {
  ...
  it('emits only -m for a persisted model the seed does not carry', () => {
    // The retirement guard lives upstream of this call; launch resolves against the
    // static seed, so an unknown id still passes through as a bare -m.
  ...
  it('drops an effort value the menu does not offer on an unseeded model', () => {
    expect(resolveAgentSessionOptionLaunch('grok', { model: 'grok-build', effort: 'none' })).toEqual(
      { args: ['-m', 'grok-build'], appliedValues: { model: 'grok-build' } }
    )
  })
  ...
  it('emits the model change as a typed slash command, never an agent picker', () => {
    ...
    const effortMid = grokEffortOption().apply.midSession!
    expect(effortMid.kind).toBe('command')
    if (effortMid.kind === 'command') {
      expect(effortMid.build('low')).toBe('/effort low')
    }
  })
})
