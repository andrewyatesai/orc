...
describe('createSequencedSetupAgentCommands', () => {
  ...
  it('wraps native Windows runners in a cmd-pinned setup and startup gate', () => {
    ...
    // Why: base64 replaced the batch `set "VAR=…"` wrapper whose quote doubling broke the marker (#8787).
    expect(result.setupCommand).not.toContain('set "ORCA_SETUP_MARKER=')
    expect(result.startupCommand).not.toContain('set "ORCA_SETUP_MARKER=')
