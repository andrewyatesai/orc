...
function planFor(platform: NodeJS.Platform, agentArgs?: string): string {
  // The startup-plan builders are rust-sole (orca-agents), so build the plan this drop has to undo
  // through the shared dispatch seam instead of a deleted TS body.
  const plan = requireOrcaDispatch('tui-agent-startup', 'buildAgentResumeStartupPlan', {
    agent: 'codex',
    providerSession: CODEX_SESSION,
    cmdOverrides: {},
    platform,
    ...(agentArgs ? { agentArgs } : {})
  }) as AgentStartupPlan | null
  if (!plan) {
