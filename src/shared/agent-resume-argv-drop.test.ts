...
// The plan builders live in Rust (rust/crates/orca-agents, reached over wasm/
// napi), which vitest cannot call from this shared suite. Reconstruct the
// command in the exact shape the builder emits: launch command + user args +
// shell-quoted resume argv from the same shared locator the builder uses.
function planFor(platform: NodeJS.Platform, agentArgs?: string): string {
  const shell = platform === 'win32' ? 'powershell' : 'posix'
  const argv = getAgentResumeArgv('codex', CODEX_SESSION)
  if (!argv) {
    throw new Error('expected a codex resume argv')
  }
  const [binary, ...resumeArgs] = argv
  const quoted = resumeArgs.map((arg) => quoteStartupArg(arg, shell)).join(' ')
  return agentArgs
    ? `${binary} ${quoteStartupArg(agentArgs, shell)} ${quoted}`
    : `${binary} ${quoted}`
}
