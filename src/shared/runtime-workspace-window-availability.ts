...
// window exists (orca-runtime.ts stamps 'available' whenever a renderer is live), so
// pairing it with a non-ready graph is the narrow "reachable, but nothing can run"
// signal: a graph-ready headless serve (#6844) and hosts that omit desktopWindowStatus
// (older builds) stay untouched.
