...
describe('scanWindowsListeningPorts', () => {
  ...
  it('drops a listening row owned by the relay process itself', async () => {
    // The self-pid filter is precisely why the fixture pids must dodge the worker: a row whose
    // pid is process.pid empties the result. Planting the collision proves the guard is real.
    const controller = new AbortController()
    execFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        host: '127.0.0.1',
