 ...
 * and other agents — the relay twin of the main process's napi detector. Callers
 * that must not materialize an agent home for a terminal that never launches one
 * need this null; `detectPiAgentKindFromCommand` collapses it to the 'pi' default.
 */
export function detectExplicitPiAgentKindFromCommand(
  ...
  // Why: classify the isolated binary through the wasm core so the OMP rule stays
  // single-sourced in Rust and an `omp` mentioned in argv cannot match.
