...
/** Result of installing Pi/OMP status into a real agent home or the OMP fallback path. */
export type MaterializePiResult = {
  /** Real agent dir when extensions were installed there. Absent for the OMP status-only fallback. */
...
export class PluginOverlayManager {
   ...
   *  home is left alone so unused agents do not recreate `~/.<agent>` (#10196).
  ...
  materializePi(
    ...
    try {
      ...
      if (!existingAgentDir && !existsSync(sourceAgentDir) && !materializeDefaultHome) {
        // Why: match the local titlebar-extension-service bare-shell OMP policy —
      ...
      return { sourceAgentDir, statusExtensionPath: extensionPath }
    } catch (err) {
      process.stderr.write(
