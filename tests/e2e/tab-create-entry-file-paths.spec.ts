...
test('new-tab file results prioritize the filename and reveal the full path on hover', async ({
  ...
  // Not the placeholder/aria-label: that copy is translated and has already drifted.
  // aria-controls points at the results listbox id, which is structural and unlocalized.
  const input = orcaPage.locator(
    'input[role="combobox"][aria-controls="tab-create-entry-results"]'
  )
