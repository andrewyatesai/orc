...
// Why: after a load fails the toolbar reload button also takes the accessible name
// "Retry", so a slot-wide name match hits two elements. Narrow to the overlay's
// button by its visible label — the toolbar one is icon-only.
