...
/** Texts of the optimistic bubbles the chat list actually renders (proves the
 *  retirement reaches the rendered tail, not just `pending`). */
...
describe('useMobileNativeChatDrafts glued pending sends', () => {
  ...
  it('keeps and renders both bubbles when the matching row predates the sends', async () => {
    ...
    await mount(history)
    rapidSend('run the tests', 'again')
    // No new row after the captured tail, so nothing can glue them.
  ...
  it('keeps both bubbles when an older turn spells them out', async () => {
    const history = [userTurn('m1', 'fix the bug', 1000), assistantTurn('m2', 'fixed', 2000)]
    await mount(history)
  ...
  // The claim-based exact path still clears each bubble on its own echo — glue
  // only adds a fallback, it must not disturb ordinary landings.
  it('still retires each bubble on its own when the sends do not glue', async () => {
    const history = [assistantTurn('m1', 'ready', 1000)]
    await mount(history)
    rapidSend('alpha', 'beta')
    const separate = [...history, userTurn('m2', 'alpha', 5000), userTurn('m3', 'beta', 6000)]
