...
describe('revealMobileSourceControlSessionDiff', () => {
  ...
  it('times out when the opened tab never appears in the snapshot', async () => {
    vi.useFakeTimers()
    const sendRequest = vi
      .fn<RpcClient['sendRequest']>()
      .mockResolvedValue(success({ tabs: [{ id: 'agent-tab', type: 'terminal' }] }))

    const reveal = revealMobileSourceControlSessionDiff(options(sendRequest))
    await vi.advanceTimersByTimeAsync(300 + 600 + 900)

    await expect(reveal).resolves.toBe('timeout')
    expect(sendRequest).not.toHaveBeenCalledWith('session.tabs.activate', expect.anything())
  })
