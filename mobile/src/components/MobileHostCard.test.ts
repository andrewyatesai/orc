...
describe('MobileHostCard', () => {
  ...
  it('announces the connection path and worktree counts without visual separators', async () => {
    const consoleError = suppressRendererDeprecation()
    await act(async () => {
      renderer = create(
        createElement(MobileHostCard, {
  ...
  it('includes visible offline recovery guidance in the navigation label', async () => {
    ...
    expect(
      renderer!.root
        .findAllByType('Text')
        .some((node) => node.children.includes('Update desktop Orca and sign in to connect from anywhere'))
    ).toBe(true)
  })
})
