...
// Boots the in-WebView host IIFE and drives external mouse/trackpad `wheel`
// events at the terminal surface. Regression guard: wheel used to fall through
// to the engine, which scrolls its hidden viewport or emits cursor keys via
// onData that the mobile query-reply gate drops — so an indirect pointer moved
// nothing inside the terminal and nothing reached the PTY. The surface wheel
// handler must reuse the touch path's scroll router.
...
function makeTerminal(
  ...
  const terminal = {
    ...
    rows: 24,
    options: { fontSize: 13 },
    // Why: init swaps surfaces (and terminals) several times before it commits.
    // Share one modes object so a test's mutation reaches whichever term is live.
    modes,
    element: { scrollWidth: 320, scrollHeight: 360 } as unknown as HTMLElement | null,
    _core: {
...
describe('terminal WebView external pointer wheel scrolling', () => {
  ...
  beforeEach(() => {
    ...
    webWindow.Terminal = function () {
      const terminal = makeTerminal(buffer, scrollLines, modes)
  ...
  it('turns an alternate-screen wheel scroll into cursor keys', () => {
  ...
  it('claims the wheel so the engine does not double-apply it', () => {
    buffer = { baseY: 20, type: 'normal', viewportY: 20 }
    boot()
