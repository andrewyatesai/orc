...
type DialableCredential = MobileRelayCredentialBundle['current']
...
export async function selectDialableRelayCredentials(args: {
  ...
  bundle: MobileRelayCredentialBundle | null
  credentials: DialableCredential[]
}> {
  ...
  // Why: no dialable credential anywhere must arm a slow reprobe, never park —
  // a later durable write (re-pair, rotation) is the only path back for a
  // relay-only phone, so recovery has to keep re-reading on its own cadence.
  args.controller.armCredentialReprobe()
