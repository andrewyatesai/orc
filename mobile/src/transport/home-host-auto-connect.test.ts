...
describe('home host auto-connect', () => {
  ...
  it('breaks lastConnected ties on id so the subset is stable', () => {
    const hosts = [host('b', 5), host('a', 5), host('c', 5), host('d', 4)]

    expect(selectHomeAutoConnectHostIds(hosts)).toEqual(['a', 'b', 'c'])
  })
