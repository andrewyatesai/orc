...
// The escalated card copy only shows in the 'reconnecting' phase — during a backoff
// cycle the state oscillates connecting↔reconnecting, and the long reconnecting dwell
// is what the user reads. Assert against that state so the check does not race which
// side of a dial the clock happens to land on.
function escalatedLabel(client: ReturnType<typeof connect>, endpoint: string): string {
  return verdictDisplayLabel(
    classifyConnection({
      state: 'reconnecting',
      reconnectAttempts: client.getReconnectAttempt(),
...
describe('foregrounding a phone that was suspended mid-dial', () => {
  ...
  it('clears the escalated label once the foreground redial lands', async () => {
    ...
    // Four failed dials is past WARNING_ATTEMPTS, so the card's reconnecting phase
    // reads "Can't connect — check Tailscale" rather than a neutral "Connecting…".
    expect(escalatedLabel(client, TAILSCALE_ENDPOINT)).toBe("Can't connect — check Tailscale")
    await advanceUntilDialing(client)
    const doomed = latest()
  ...
  it('keeps escalating when the user checks back repeatedly during a real outage', async () => {
    ...
    expect(escalatedLabel(client, TAILSCALE_ENDPOINT)).toBe("Can't reach desktop — check Tailscale")
