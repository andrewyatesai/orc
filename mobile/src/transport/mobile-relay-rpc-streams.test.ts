...
describe('MobileRelayRpcStreams failure parity', () => {
  ...
  it('does not emit a failure after the caller cancels a queued stream', async () => {
    ...
    const sendFrame = vi.fn((frame: Frame) => {
    ...
    // Cancel still emits the worktree-keyed teardown, but the queued subscribe
    // frame must never reach the host.
    expect(methods).toEqual(['session.tabs.unsubscribe'])
  })
