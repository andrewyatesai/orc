...
import { RpcConnectWaitQueue } from './rpc-connect-wait-queue'
...
export function connect(
  ...
  const connectWaits = new RpcConnectWaitQueue()
  ...
  function waitForConnected(timeoutMs?: number): Promise<void> {
    ...
    return connectWaits.wait(timeoutMs)
  }
  ...
  function scheduleReconnect() {
    ...
    if (pastGiveUpCap) {
      ...
      connectWaits.rejectAll('Connection retry limit reached')
    } else {
