 ...
 *
 * Time-to-first-terminal lane (`pnpm bench:first-terminal`): waits on the
 * 'renderer-first-terminal-frame' milestone — the first presented frame of the
 * first restored pane, which lands AFTER workspace-ready (aterm engine cold
 * boot). Reported as totalToFirstTerminalFrame and
 * workspaceReadyToFirstTerminalFrame. The event only fires when a session
 * restores a terminal, so the lane runs --state-profile restored-local-tabs;
 * 8 session tabs models a realistic small daily session without letting tab
 * fan-out drown the single-pane cold-boot signal.
 *
