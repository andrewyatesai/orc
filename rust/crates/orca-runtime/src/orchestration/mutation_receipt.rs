//! Mutation receipts (schema v11): the durable idempotency ledger keyed by
//! `(caller_fingerprint, request_id)`. A retried mutating RPC replays its
//! recorded result instead of applying the mutation twice, and a caller that
//! changes the payload under the same request id is rejected rather than
//! silently accepted.
//!
//! Ported from the reference branch's `orchestration/mutation_receipt.rs` (v22),
//! adapted to this schema. Unlike the reference — which imports the row struct
//! from a `super::rows` module this schema does not have — the row struct,
//! its column list, and `row_to_mutation_receipt` stay LOCAL here, matching this
//! store's convention (capability.rs keeps its own types too).
...
/// Ledger caps: bound the durable table so an abandoned caller cannot grow it
/// without limit. Aged-out completed rows and the oldest completed rows are
/// trimmed first; a ledger full of unresolved rows refuses new admissions.
const MUTATION_RECEIPT_MAX_ROWS: i64 = 10_000;
const MUTATION_RECEIPT_MAX_AGE_DAYS: i64 = 30;

/// TS `MutationReceiptRow` — one row of the idempotency ledger.
...
pub const MUTATION_RECEIPT_COLUMNS: &str =
    "caller_fingerprint, request_id, method, payload_hash, state, receipt, created_at, updated_at";

pub(crate) fn row_to_mutation_receipt(row: &SqlRow<'_>) -> rusqlite::Result<MutationReceipt> {
    Ok(MutationReceipt {
...
/// The identity of one caller mutation. `payload_hash` is computed by the shim
/// (the store treats it as an opaque fingerprint of the canonicalized params).
...
impl OrchestrationDb {
    ...
    /// mismatched `method`/`payload_hash` for an existing key must fail, not
    /// overwrite.
    ...
    fn begin_mutation_receipt_in_transaction(
        ...
        if let Some(existing) = self.get_mutation_receipt(&key.caller_fingerprint, &key.request_id)?
        {
            ...
            let disposition = match existing.state.as_str() {
                ...
                // rather than guessing a disposition the caller would act on.
                other => {
    ...
    fn ensure_mutation_receipt_capacity(&self) -> Result<(), StoreError> {
        let conn = self.db.connection();
        conn.execute(
            "DELETE FROM mutation_receipts
...
mod tests {
    ...
    #[test]
    fn complete_is_idempotent_and_restamps_the_result() {
        let db = OrchestrationDb::open_in_memory().unwrap();
        let k = key("req-1", "hash-1");
        db.begin_mutation_receipt(&k).unwrap();
        db.complete_mutation_receipt(&k, r#"{"v":1}"#).unwrap();
        // Complete is not gated on state = 'pending', so a re-complete wins.
    ...
    fn discard_releases_only_a_pending_slot() {
        ...
        db.discard_pending_mutation_receipt("peer-a", "req-1").unwrap();
        assert_eq!(db.get_mutation_receipt("peer-a", "req-1").unwrap().unwrap().state, "completed");
