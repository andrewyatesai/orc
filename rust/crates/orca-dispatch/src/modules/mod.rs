...
//! registry. Two modules are the exception: `orchestration-store` needs rusqlite/
//! orca-runtime, and `nacl-box` would pull curve25519/salsa20/poly1305 into the
//! relay wasm for no caller. Both stay local to orca-parity, which layers them
//! over this aggregate dispatch. Tooling that reads dispatch coverage must read
//! BOTH registries — reading only this one reports those two as unverified,
//! which is backwards: they are parity-only oracles by design.
